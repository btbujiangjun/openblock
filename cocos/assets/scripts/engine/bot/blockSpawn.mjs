/* 自动生成 —— 请勿手改。源：web/src/bot/blockSpawn.js
...
import { getAllShapes, getShapeCategory, isSpecialShapeId } from '../shapes.mjs';
import { GAME_RULES } from '../gameRules.mjs';
import { _sanitizeShapeArr, _pickFallbackSafe } from './spawnSanitize.mjs';
export { _sanitizeShapeArr, _pickFallbackSafe };
...
import { _tryInjectSpecial } from './specialInjection.mjs';
export { _tryInjectSpecial };
import { argmaxAlign, pickBestAligned } from './alignedPick.mjs';
...
import { alignmentMultiplier } from '../difficultyRelativity.mjs';
 ...
 */
export const SPECIAL_SHAPE_WEIGHTS = {
    '1x2': 3, '2x1': 3,
    '1x3': 2, '3x1': 2,
...
export const RELIEF_FILL_FLOOR_URGENT = _SB.reliefFillFloorUrgent ?? 0.25;
export const RELIEF_FILL_FLOOR_MILD = _SB.reliefFillFloorMild ?? 0.35;
...
export const RELIEF_HOLE_FILL_MIN = _SB.reliefHoleFillMin ?? 2;
 ...
 */
export function _resolveSpecialPools(ctx) {
...
export function countLegalPlacements(grid, shapeData) {
    let c = 0;
    const n = grid.size;
    for (let y = 0; y < n; y++)
        for (let x = 0; x < n; x++)
 ...
 */
export function bestMultiClearPotential(grid, shapeData) {
    const n = grid.size;
    let best = 0;
 ...
 */
export function bestHoleReduction(grid, shapeData, currentHoles) {
 ...
 */
export const DUP_INJECT_CONFIG = Object.freeze({
    HIGH_THRESHOLD:    0.65,
 ...
 */
export function _tryInjectDuplicates(triplet, chosenMeta, hints, ctx, grid, opts = {}) {
 ...
 * @param {() => number} [rng]
 */
export const pickWeighted = (pool, rng) => {
