/* 自动生成 —— 请勿手改。源：web/src/evaluation/evaluationHost.js
...
import { evaluateRound } from './roundQuality.mjs';
import { buildSessionEvalRecord } from './sessionEvaluator.mjs';
import {
    ...
    setLedgerOutcome,
    patchLedgerMeta,
} from './evaluationLedger.mjs';

export function evalOnSessionStart(host, meta = {}) {
...
export function evalOnSpawn(host, shapes) {
...
export function evalOnPlace(host, dockIndex, pos, linesCleared) {
    const led = host.evalLedger;
    if (!led) return;
    try {
...
export function evalCloseRound(host) {
    const led = host.evalLedger;
    if (!led) return;
    try {
...
export async function evalOnGameOver(host, outcome) {
    const led = host.evalLedger;
    if (!led) return;
    try {
        if (host.evalActiveSpawnIdx >= 0) {
