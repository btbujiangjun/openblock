/* 自动生成 —— 请勿手改。源：web/src/adaptiveSpawn.js
...
import { applyDecisionTable, applyPriorityLadder } from './lib/decisionTable.mjs';
import {
...
import { buildPlayerAbilityVector } from './playerAbilityModel.mjs';
import { analyzeBoardTopology } from './boardTopology.mjs';
import { getAllShapes } from './shapes.mjs';
import { analyzePerfectClearSetup } from './bot/blockSpawn.mjs';
import { getLifecycleMaturitySnapshot } from './retention/playerLifecycleDashboard.mjs';
import { getCachedLifecycleSnapshot } from './lifecycle/lifecycleSignals.mjs';
/* v1.48：winback 保护包接入；通过 lifecycleOrchestrator 包装层避免直接依赖
 * retention 模块（保持单向依赖：spawn 层 → lifecycle 编排层 → retention 模块）。 */
import { getActiveWinbackPreset } from './lifecycle/lifecycleOrchestrator.mjs';
...
import { applyPeogSpawnHintsCap } from './spawn/peog.mjs';
/* §4.17/§2.10 难度相对论：体感→客观目标 b* 反解（影子，不改 stress 主线）。
 * §O1：resolveRelativityIntent 把"以什么力度参与"按相位集中决策，与 bypass 互补。 */
import { solveObjectiveTarget, resolveRelativityIntent, RELATIVITY_INTENT } from './difficultyRelativity.mjs';
...
import { applySpawnPrior, applyRelativityShapePrior } from './bot/spawnPriors.mjs';
export { applySpawnPrior, applyRelativityShapePrior };
...
import {
    ...
    SPAWN_PARAM_KEYS,
} from './bot/pbCurve.mjs';
export { derivePbCurve, DEFAULT_SPAWN_PARAMS_PB_CURVE, SPAWN_PARAM_KEYS };
...
} from './bot/liveGeometrySignals.mjs';
/* v1.71：deriveDelightTuning 抽至 ./bot/delightTuning.js。 */
import { deriveDelightTuning } from './bot/delightTuning.mjs';
import {
 ...
 * @returns {boolean}
 */
export function deriveBottleneckSignal(ctx, profile) {
