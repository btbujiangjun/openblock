/* 自动生成 —— 请勿手改。源：web/src/spatialPlanning.js
...
import { getAllShapes, getRegularShapes, getShapeCategory } from './shapes.mjs';

export const SPATIAL_PLANNING_VERSION = 1;

/** 廉价 RL 子向量维度（state 标量段扩增量）。 */
export const SPATIAL_PLANNING_FEATURE_DIM = 3;

/** 默认配置（可被 game_rules.json `spatialPlanning` 覆盖）。 */
export const DEFAULT_SPATIAL_PLANNING_CONFIG = Object.freeze({
    boardSize: 8,
 ...
 * @returns {number}
 */
export function shannonEntropy(counts, normDenom) {
    let total = 0;
 ...
 */
export function scanEmptyRegions(grid, cfg) {
 ...
 */
export function spatialPlanningFeatures(grid, cfg) {
 ...
 * }}
 */
export function computeVocabularyMobility(grid, opts = {}) {
 ...
 */
export function computeTopologyDelta(before, after, cfg) {
    const c = mergeConfig(cfg);
 ...
 * @returns {object}
 */
export function computeSpatialPlanning(grid, opts = {}) {
    const cfg = mergeConfig(opts.cfg);
    const scan = scanEmptyRegions(grid, cfg);
 ...
 * @returns {number}
 */
export function computeSpatialPlanningScore(input = {}, cfg) {
    const c = mergeConfig(cfg);
