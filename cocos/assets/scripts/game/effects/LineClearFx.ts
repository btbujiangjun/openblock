...
export class LineClearFx extends Component {
    ...
    play(result: ClearResult, skin: Skin, opts: { perfectClear?: boolean } = {}): void {
        ...
        const baseMs = perfectClear ? 1050 : isCombo ? 780 : isDouble ? 620 : 500;
        const durationS = (reduced ? baseMs * 0.6 : baseMs) / 1000;
        // 强度系数：perfect/combo/double 在 web 上是通过 perCell 粒子量 + 全屏闪光体现的，
        // 这里给 LineClearFx 本身也按档位放大「亮度 + 半径」，让单格辉光不被 1.05s 长窗口稀释。
        const intensity = perfectClear ? 1.35 : isCombo ? 1.20 : isDouble ? 1.08 : 1.0;
