...
export RL_ENTROPY_COEF_MIN="${RL_ENTROPY_COEF_MIN:-0.025}" # 0.02→0.025：11.5h 长跑见熵 2.22→1.81（-18%）持续下降趋势，
                                                       # 抬高下限保留残局探索；配合 entropy_target=2.0 防过早 commit
# ── v13 防退化栈（针对「得分越来越低」根因，全部可被外部 env 覆盖）──
export RL_TARGET_KL="${RL_TARGET_KL:-8.0}"             # 5.0→8.0：11.5h 长跑后 ppoE 从 4.0 跌回 2.49（早停率↑），
                                                       # 真实 KL 在 1.7-2.5 偏置后但 P95 仍触 5.0。放到 8.0 让 4-epoch 重新跑满，
                                                       # 仍在策略真发散（>8）时早停。本质同上 5.0→8.0：
...
export RL_AUTO_PERIODIC_RESTART_EVERY="${RL_AUTO_PERIODIC_RESTART_EVERY:-15000}"  # P2 · 每 N 局主动 exit 99 由后端限速 resume，避免长跑内存碎片不可逆累积
...
# HIGH=0.7 → MPS 最多占 ~26G；LOW=0.5 → 跌破时尝试 trim 归还 driver
...
echo "==> RL OOM 防护: empty_cache_every=${RL_EMPTY_CACHE_EVERY} replay_max_steps=${RL_REPLAY_MAX_STEPS} periodic_restart_every=${RL_AUTO_PERIODIC_RESTART_EVERY}"
