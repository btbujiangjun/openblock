...
export RL_VALUE_COEF="${RL_VALUE_COEF:-0.5}"            # 1.25→0.5：防价值头发散。conv-shared 共享主干下，A+B 放开 PPO 到 4 epoch 后，
                                                       # value_coef=1.5 使价值梯度(loss_value 一度冲到 745，×1.5≈策略项 70 倍)主导并污染主干，
                                                       # 引发 loss_value 正反馈爆炸→策略崩塌→分数从~1300 跌到~900。价值梯度预算≈value_coef×epochs，
                                                       # 实测稳定基线≈1.5；取 0.5 配 3 epoch(0.5×3=1.5)回到该预算，仍保留 3× 样本利用率。
