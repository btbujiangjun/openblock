...
const { DIFFICULTY_VECTOR_DIMS } = require('./spawnStepDifficulty');

/** θ⃗ 维度顺序（与 difficultyVec 一致，SSOT）。 */
const LATENT_DIMS = DIFFICULTY_VECTOR_DIMS;

const LATENT_ABILITY_VERSION = 1;
...
/** 新建初始 θ⃗ 状态（各维先验 μ/σ）。 */
function createLatentState(cfg) {
 ...
 */
function mapAbilityToObservation(ability) {
 ...
 */
function updateLatentState(state, observation, cfg) {
    const c = mergeLatentCfg(cfg);
...
function latentConfidence(state, cfg) {
    const c = mergeLatentCfg(cfg);
 ...
 */
function getCalibrationVector(state, fullCfg) {
...
/** 完整可观测快照（面板 / 回放帧 / 诊断用）。 */
function snapshotLatent(state, cfg) {
    const base = state && state.dims ? state : createLatentState(cfg);
    const mu = {};
    const sigma = {};
    for (let i = 0; i < LATENT_DIMS.length; i++) {
...
module.exports = { createLatentState, deserializeLatent, getCalibrationVector, LATENT_ABILITY_VERSION, LATENT_DIMS, latentConfidence, mapAbilityToObservation, serializeLatent, snapshotLatent, updateLatentState };
