...
function isCampaignActive(campaign, nowTs = Date.now()) {
...
function matchesAudience(campaign, ctx = {}) {
 ...
 * @param {number} nowTs
 */
function getActiveCampaigns(config, ctx = {}, nowTs = Date.now()) {
...
function getTopCampaign(config, ctx = {}, nowTs = Date.now()) {
    return getActiveCampaigns(config, ctx, nowTs)[0] || null;
}

module.exports = { getActiveCampaigns, getTopCampaign, isCampaignActive, matchesAudience };
