...
/** 按未游玩天数返回召回层级。 */
function classifyWinbackTier(daysSinceLastPlay) {
...
/** 是否应触发召回礼包（none 层不触发）。 */
function shouldOfferWinback(daysSinceLastPlay) {
...
function resolveWinback(lastPlayTs, nowTs = Date.now()) {
...
module.exports = { classifyWinbackTier, daysSince, resolveWinback, shouldOfferWinback, WINBACK_TIERS };
