 ...
 */
async function verifyEd25519({ messageBytes, signatureBytes, publicKeyBytes }) {
 ...
 */
async function verifyHmacSha256({ messageBytes, signatureBytes, keyBytes }) {
    const subtle = _subtle();
    if (!subtle) return false;
    try {
        const key = await subtle.importKey(
 ...
 */
async function verify({ payload, signature, algorithm, key }) {
 ...
 */
function createVerifier({ algorithm, key }) {
    return async ({ payload, signature }) => verify({ payload, signature, algorithm, key });
}

/* ---------- 测试辅助 ---------- */
const _internal = { _b64ToBytes, _bytesToB64, _utf8Bytes };

module.exports = { _internal, canonicalJson, createVerifier, verify, verifyEd25519, verifyHmacSha256 };
