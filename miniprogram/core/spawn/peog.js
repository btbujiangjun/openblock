 ...
 */
function estimateConstructiveYield(candidate) {
 ...
 */
function buildPeogState(profile, ctx, warmRunState) {
 ...
 */
function consumePeogOnPlace(state, ctx, scoreDelta) {
    if (!state) return;
 ...
 */
function applyPeogSpawnHintsCap(enhancedConfig, peogState) {
 ...
 */
function applyPeogYieldCap(candidates, peogState) {
 ...
 * @returns {boolean}
 */
function shouldShortCircuitConstructive(peogState, kind) {
...
function formatPeogTrace(state) {
    if (!state) return '';
...
const PEOG_INTENSITY_RANK = Object.freeze({
    peog_mild: 1,
    peog_strong: 2,
});

const PEOG_BYPASS_REASONS = Object.freeze([
    'disabled',
    'rollout_out',
    'low_pb',
    't1_newbie',
...
module.exports = { applyPeogSpawnHintsCap, applyPeogYieldCap, buildPeogState, consumePeogOnPlace, estimateConstructiveYield, evaluatePeogActive, formatPeogTrace, PEOG_BYPASS_REASONS, PEOG_INTENSITY_RANK, shouldShortCircuitConstructive };
