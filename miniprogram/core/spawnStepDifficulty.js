...
const SPAWN_STEP_DIFFICULTY_VERSION = 1;

/** 5 档难度桶（替代「题目级」聚合主键：按难度桶 × 算法离线聚合）。 */
const DIFFICULTY_BUCKETS = ['trivial', 'easy', 'standard', 'hard', 'extreme'];
...
const DEFAULT_STEP_DIFFICULTY_CONFIG = Object.freeze({
    boardSize: 8,
...
const DIFFICULTY_VECTOR_DIMS = ['spatial', 'combo', 'order', 'recovery', 'tempo', 'clearEff'];
 ...
 */
function projectDifficultyVector(terms, cfg) {
 ...
 * @param {object} [cfg]
 */
function isKillerShape(data, countLegal, cfg) {
    const c = mergeConfig(cfg);
    const cells = shapeCellCount(data);
 ...
 */
function classifyTriplet(shapes, ctx = {}, cfg) {
    const c = mergeConfig(cfg);
 ...
 * @param {object} [cfg]
 */
function scdScore(comboTotalCells, occupiedCount, cfg) {
    const c = mergeConfig(cfg);
 ...
 * @returns {number[]} 长度 4
 */
function spawnStepDifficultyFeatures(shapes, occupiedCount, cfg) {
    const c = mergeConfig(cfg);
...
const SPAWN_STEP_DIFFICULTY_FEATURE_DIM = 4;

/** stepDifficulty(0~1) → 5 档桶。 */
function difficultyBucket(stepDifficulty) {
 ...
 * @param {object} [cfg]
 */
function computeSpawnStepDifficulty(input = {}, cfg) {
    const c = mergeConfig(cfg);
    const {
        shapes = [],
        occupiedCount = 0,
...
module.exports = { classifyTriplet, computeSpawnStepDifficulty, DEFAULT_STEP_DIFFICULTY_CONFIG, DIFFICULTY_BUCKETS, DIFFICULTY_VECTOR_DIMS, difficultyBucket, isKillerShape, isLongBar, projectDifficultyVector, scdLevel, scdScore, shapeCellCount, SPAWN_STEP_DIFFICULTY_FEATURE_DIM, SPAWN_STEP_DIFFICULTY_VERSION, spawnStepDifficultyFeatures };
