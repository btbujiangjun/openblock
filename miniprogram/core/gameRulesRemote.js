 ...
 *
 * 用法（启动期）：
 *   const { initRemoteRules } = require('./gameRulesRemote');
...
const { GAME_RULES, _replaceRulesForRemoteSync } = require('./gameRules');
let _softDeps_userBucketing = {}; try { _softDeps_userBucketing = require('./lib/userBucketing'); } catch (_e) { /* miniprogram 不分发 lib/ 子目录，软依赖回退空骨架 */ } const { getUserBucket } = _softDeps_userBucketing;
 ...
 */
async function initRemoteRules(opts) {
...
/* 测试辅助 —— 不导出给生产路径使用。 */
function _resetRemoteRulesForTest() {
    _sessionFailures = 0;
    _inflight = null;
}
...
module.exports = { _internal, _resetRemoteRulesForTest, initRemoteRules };
