...
const { computeSpatialPlanning, computeSpatialPlanningScore } = require('./spatialPlanning');
...
let _personalizationMod = {}; try { _personalizationMod = require('./monetization/personalization'); } catch (_e) { /* miniprogram 不分发 monetization/ 子目录，软依赖回退空骨架 */ }
let _ltvPredictorMod = {}; try { _ltvPredictorMod = require('./monetization/ltvPredictor'); } catch (_e) { /* miniprogram 不分发 monetization/ 子目录，软依赖回退空骨架 */ }
let _monBusMod = {}; try { _monBusMod = require('./monetization/MonetizationBus'); } catch (_e) { /* miniprogram 不分发 monetization/ 子目录，软依赖回退空骨架 */ }
 ...
 */
const ABILITY_VECTOR_VERSION = 2;
 ...
 */
function buildPlayerAbilityVector(profile, ctx = {}) {
 ...
 */
function buildAbilityTrainingDataset(sessions) {
 ...
 */
function getPlayerAbilityModel() {
    return {
        getPersona() {
            try {
...
function getPersona() { return getPlayerAbilityModel().getPersona(); }
function getRealtimeState() { return getPlayerAbilityModel().getRealtimeState(); }
function getLTV() { return getPlayerAbilityModel().getLTV(); }

module.exports = { ABILITY_VECTOR_VERSION, buildAbilityTrainingDataset, buildPlayerAbilityVector, getLTV, getPersona, getPlayerAbilityModel, getRealtimeState };
