...
const SKIN_PREMIUM_STORAGE_KEY = 'openblock_skin_premium_v1';
...
function loadPremiumPrefs(storage) {
    try {
...
function savePremiumPrefs(storage, prefs) {
    try {
 ...
 */
function computePremiumSkinVars(skin) {
    if (!skin) return {};
 ...
 */
function isPremiumRenderEnabled({
    enabled = false,
    qualityMode = 'high',
    qualityOff = false,
} = {}) {
