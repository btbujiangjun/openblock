...
const { defaultRng } = require('../lib/seededRng');
const { getAllShapes } = require('../shapes');
const _BS_ESM = require('./blockSpawn');

/* 循环依赖处理策略（双轨）：
 *   1. ESM 端（web/cocos .mjs）：`const _BS_ESM = require('./blockSpawn')` 走 namespace
...
function _tryInjectSpecial(triplet, chosenMeta, hints, ctx, grid, fill, topo, pcSetup, scored, opts, helpers) {
...
module.exports = { _tryInjectSpecial };
