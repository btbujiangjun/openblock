...
const { getShapeById } = require('../shapes');
const {
    computeClearScore,
    detectBonusLines,
    ...
    PERFECT_CLEAR_MULT,
} = require('../clearScoring');

const NEWBIE_VILLAGE_STORAGE_KEY = 'openblock_newbie_village_v1';

const NV_COLS = 8;
const NV_ROWS = 8;
/** 与主局默认策略一致（shared/game_rules.json → defaultStrategyId = 'normal'） */
const NV_STRATEGY_ID = 'normal';
...
function loadVillageState(storage) {
    try {
...
function saveVillageState(storage, patch) {
    try {
 ...
 */
function shouldShowNewbieVillageCore({ game, storage, force = false, skip = false } = {}) {
...
function applyPiece(board, piece, origin) {
...
function isPlacementValid(board, piece, origin) {
    const [ox, oy] = origin;
 ...
 */
function pickSmartSnap(board, piece, aim, snapRadius = 1) {
    if (!aim) return null;
    const [ax, ay] = aim;
...
function scorePlacement(filledBoard, comboCount) {
...
module.exports = { applyPiece, breakdownText, computeClears, deriveNextComboCount, emptyNvBoard, ICON_BONUS_LINE_MULT, isPlacementValid, loadVillageState, NEWBIE_VILLAGE_STORAGE_KEY, NV_COLS, NV_PALETTE, NV_ROWS, NV_STRATEGY_ID, PERFECT_CLEAR_MULT, pickSmartSnap, saveVillageState, SCENARIO, scorePlacement, shapeCells, shouldShowNewbieVillageCore, toGridLike };
