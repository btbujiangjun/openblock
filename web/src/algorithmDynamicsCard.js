...
export function renderDifficultyRelativityCard(insight) {
    ...
    if (r.targetGap != null) meta.push(`偏差 ${_fmt(r.targetGap, 2)}`);
    if (r.candidatesConsidered != null) meta.push(`候选 ${r.candidatesConsidered}`);
    /* 相位化对齐预算 → 人话短语徽标（"相对论暂停 / 只调形状池 / 完整个性化"）。 */
    ...
    /* b* 早期上界触发徽标——直接说"前期不喂偏难"。 */
    if (r.earlyPhaseCapHit === true) {
        const capTip = _attr('高分玩家前期不喂偏难：当前 d* 偏低（局初），即便你 θ⃗ 能力高，'
    ...
    /* PEOG bottleneck 让位累计——徽标显示"PB 加压中（抗抖动）"+ 计数。 */
    if (r.peogYieldHits && (r.peogYieldHits.bottleneckHits > 0 || r.peogYieldHits.nearMissHits > 0)) {
        ...
        const pTip = _attr(`PB 加压窗口抗抖动：PEOG（PB 早期超越守卫）正在压制 b*，`
