...
export const CONFIG = {
    ...
    /**
     * 触摸跟手系数（v1.61.15，安卓 / iOS / 任意触屏统一生效）：触摸是「直接操控」，
     * 走 1:1 绝对定位（相对抓取点缩放），不套用鼠标的指针加速——加速会让幽灵块随采样
     * 噪声放大漂移、表现为「过于敏感 / 乱飘 / 几乎无法操作」。
     *   = 1.0 → 完全 1:1，幽灵块位移严格等于手指位移（标准休闲游戏响应速度，默认）
     *   < 1.0 → 轻微减速（手指多走一点、落点更稳），但长距离会与手指产生可见滞后，慎调
     *   > 1.0 → 放大（不推荐，会重新引入敏感/失控）
     * 注：DRAG_TOUCH_GAIN / _GAIN_MIN / _SPEED_* 那套速度感知曲线现仅鼠标使用，触摸不再消费。
     */
    DRAG_TOUCH_TRACK_GAIN: 1.0,
