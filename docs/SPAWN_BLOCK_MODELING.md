...
1. **形状级特征与先验排序**  
   对每个可放置形状计算：`gapFills`（能否补缺口触发消行）、`placements`、`multiClear`、`holeReduce`（高填充且空洞多）、以及品类与策略权重 `weight`。默认按 `gapFills`、`multiClear` 排序，供后续选「消行位」。

2. **两阶段构造 triplet**  
   - **阶段 1**：从「能消行」子集按 `clearGuarantee`（及 combo 加强后的 `effectiveClearTarget`）占坑；高 `multiClearBonus` 时排序偏向「多消 + 缺口」综合分。  
   - **阶段 2**：对剩余槽位做 **加权抽样** `pickWeighted(augmentPool(...))`，权重为多层乘子（机动性、空洞、多消、combo、节奏、`sizePreference`、多样性惩罚、里程碑等）。

3. **拒绝采样循环**  
   若 triplet 违反机动性或可解性，则 `attempt++` 重试，最多 `MAX_SPAWN_ATTEMPTS`；仍失败则走简化兜底路径。
...
- `gapFills` / `countGapFills`：与即时消行强相关，是阶段 1 的主键。
- `placements`：合法位置计数，进入 \(\log\) 型机动性奖励。
- `multiClear`：`previewClearOutcome` 上的行列消除数上界。
...
\[
d^\* = \mathrm{clamp}\bigl(
0.3 + 0.5\,\mathrm{skill} - 0.2\,\mathrm{frustration} - 0.15\,\mathrm{stress} + 0.1\,\mathrm{fill},\, 0,\,1
\bigr)
\]
...
| 三层架构说明 | `docs/SPAWN_ALGORITHM.md` |
| 自适应系统设计 | `docs/ADAPTIVE_SPAWN.md` |
