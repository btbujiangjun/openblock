...
| 2 | **形状库扩充**（pentomino） | ★★ 强 | 加任一 pentomino 即可让奖励显著下行；加两个在 `h=2, p=0` 下直接训练不收敛；**T-pentomino 单独造成最大减速** | ✓ v1.60.0：28 → 40 形状（新增 12 个 2-3 格形状），按"加减压策略"做 gate + 加权（详见 [ADAPTIVE_SPAWN §10.7](./ADAPTIVE_SPAWN.md#107-形状池扩展v1600)） |
...
v1.56 在 `blockSpawn.js` 的 `augmentPool` 末尾追加一条**远征段多消潜力倾斜**规则：当 `spawnHints.farFromPBBoostActive === true`（PB 距离段 D0）且候选块 `s.multiClear >= 2` 时，权重乘 `1.15`。
...
**调制表（`web/src/lifecycle/lifecycleStressCapMap.js`，全仓 single source of truth；v1.55 补全为 25 格）**：
...
> v1.55 之前，S0·M1+、S2·M4、S3·M0 等 8 个组合未定义（表中显示 `—`），`getLifecycleStressCap`
> 返回 `null`，raw stress 直通形成"死键"问题；v1.55 按行内单调递增、列内 S0/S4 弱 / S2/S3 强原则
> 补全为 25 格，消除了该 bug。
