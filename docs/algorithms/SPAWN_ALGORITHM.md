...
> **评估入口**：出块调参前先跑 [出块评估与可视化工具](./SPAWN_EVALUATION.md)。
> 它提供 `npm run spawn:eval` CLI 与 `web/spawn-eval.html` 页面，用随机 / 贪心消行 /
> 生存优先三类 bot 建立公平性、奖励节奏、兜底率和组合重构收益的基线。
...
> **一图入门**：下图是 `generateDockShapes` 的 9 层端到端流水线全览，对应本文
> §2.5 的 5 阶段分解（盘面感知 → 评分 → 优先选拔 → 加权补齐 → 约束验证 → 注入优化 → 输出 → 染色）：
>
> ![出块算法架构图：9层流水线（输入层→染色层）](./assets/spawn-architecture.png)
>
> 图中各层与本文章节的对应：**层 0–1**（输入 + 盘面感知）→ §2.2；**层 2–3**（评分 + 优先选拔）→ §2.5.1 阶段 1–2；**层 4**（加权补齐）→ §2.5.2 表 B；**层 5**（约束验证）→ §2.5.2 表 C；**层 6**（注入优化）→ §2.5 阶段 5；**层 7**（输出）→ §2.5.1 阶段 5；**层 8**（染色）→ §2.1。
...
### 2.2 `multiLineTarget`、收紧的 `rhythmPhase`、局间 `warmup`
...
（`web/src/grid.js` + `web/src/bot/blockSpawn.js`，小程序 / `rl_pytorch` / `rl_mlx` 的 `Grid` 同步）：
...
| `deriveMultiClearBonus` 基底 | Idle 分支为 **0.22**（`game_rules.adaptiveSpawn.delight.baseMultiClearBoost` 同步） |
...
| **`scoreMilestone`** | `ctx.scoreMilestone && gapFills>0` → `w *= 1.3` | `:870-872` | 接近 PB 50/75/90% 时偏好能消行块 |
...
### 2.5.5 scoreMilestone 桥接

此前，`blockSpawn.js:870-872` 的 `ctx.scoreMilestone && s.gapFills > 0 → w *= 1.3` 加权分支属于 **dead branch**：
...
| 2 | **形状库扩充**（pentomino） | ★★ 强 | 加任一 pentomino 即可让奖励显著下行；加两个在 `h=2, p=0` 下直接训练不收敛；**T-pentomino 单独造成最大减速** | ✓ 28 → 40 形状（新增 12 个 2-3 格形状），按"加减压策略"做 gate + 加权（详见 [ADAPTIVE_SPAWN §10.7](./ADAPTIVE_SPAWN.md#107-形状池扩展)） |
...
> **完整启示与对"挑战自我"主线的应用**：见 [ADAPTIVE_SPAWN §10.6 外部实证基线](./ADAPTIVE_SPAWN.md#106-外部实证基线sgaz--tetris-block-puzzle) 与 [最佳分追逐策略 §5.z 规则层调控（未来方向）](../player/BEST_SCORE_CHASE_STRATEGY.md#5z-基于-sgaz-实证的规则层调控未来方向)。

## 2.7 PB 距离段倾斜

> 与 [BEST_SCORE_CHASE_STRATEGY §5.α §2.5](../player/BEST_SCORE_CHASE_STRATEGY.md) 和 [ADAPTIVE_SPAWN §13](./ADAPTIVE_SPAWN.md#13-v156-pb-段差异化机制farfrompbboost--pbextremeorderboost) 双向引用。
...
```js
// blockSpawn.js 形状权重打分末尾
if (farFromPBBoostActive && s.multiClear >= 2) {
    w *= 1.15;
}
```

**与上层 `spawnHints` 调控的关系**：

| 层 | 远征段（D0）注入 | 实施位置 |
|----|------------------|----------|
| L2 数值 | `multiClearBonus ≥ 0.45`、`iconBonusTarget ≥ 0.30`、`clearGuarantee ≥ 2` | `adaptiveSpawn.js §2.1` |
| L3 形状 | `s.multiClear >= 2` 的候选 ×1.15 | `blockSpawn.js §2.5`（本节） |
| L3 特效 | 多消 / perfect / bonusLines 振幅 ×1.3 | `game.js playClearEffect()` |

三层一致地把"远征送爽"从数值意图、形状选择到视觉反馈贯通，让 D0 段开局即可感受到"系统在帮我开局"。

**与里程碑加权（×1.3）的克制**：

`farFromPBBoostActive` 仅给 `multiClear >= 2` 的候选 ×1.15，明显低于里程碑加权（gapFills > 0 时 ×1.3）；当两者同帧（例如玩家刚跨过 50% × bestScore 又恰好在 D0 段）时，乘子叠加为 ×1.495，仍在 augmentPool 数值健康区间内（典型加权乘子上限 ×2.5）。
...
对每个候选形状，遍历所有合法放置位，用 `grid.previewClearOutcome` 模拟放置后的消行数，返回最大值。不再以 `gapFills > 0` 为前置条件才算 `multiClear`（见 §2.3）。当该形状 `multiClear ≥ 2` 时在权重中获得额外加成：
...
### 4.3 多线目标 `multiLineTarget`（0 / 1 / 2）
...
> 格式：`cap / adj`。cap = stress 硬上限；adj = cap 处理后的整体偏移（负值=减压，正值=加压）。
...
此前，S0·M1+、S2·M4、S3·M0 等 8 个组合未定义（表中显示 `—`），`getLifecycleStressCap` 返回 `null`，raw stress 直通形成"死键"问题；现已按行内单调递增、列内 S0/S4 弱 / S2/S3 强原则补全为 25 格。

**两个维度的影响幅度**：

- **band 移动**（同 stage）：M0→M4 → cap 抬升 0.13–0.23 → 对应本文 §7 的 10 档 difficulty profile 的 **2–4 档**差距；
- **stage 移动**（同 band）：S0/S4（保护期）vs S2/S3（挑战期）在同 band 下 cap 差 0.10–0.20。

> 完整 25 格矩阵、SkillScore 计算公式、三条接入路径的详细说明见
> [自适应出块 §10.10.10](./ADAPTIVE_SPAWN.md#101010-生命周期--成熟度如何作用于出块算法)。
...
详细推导见 [`ADAPTIVE_SPAWN.md` §5.1.2](./ADAPTIVE_SPAWN.md#512-生命周期--成熟度-stress-调制)。
