...
| 观测编码（实现事实来源） | **`featureEncoding.stateDim = 201`**（v12）：62 维标量（25 结构 + 19 颜色 + 4 单步难度 + 3 策略 one-hot + **5 RoR arc + 6 spawnIntent 条件 token**）+ 64 棋盘 + **75** dock 掩码；**$\phi\in\mathbb{R}^{216}$**。训练每局随机 `strategyId` + 按 `samplingProb=0.6` 随机注入 (arc, intent) 风格条件；推理用 UI 所选难度，arc/intent 显式指定或保持 None。 | 固定维度、可微近似 |
