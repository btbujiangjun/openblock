...
- [用户实时状态历史序列分析](./REALTIME_STATE_HISTORY_ANALYSIS.md) —— 基于 `move_sequences.frames[*].ps` 的历史实时状态分布、互操作关系、stress 分量贡献与已落地优化项；可用 `npm run spawn:realtime-tune` 重新分析并生成/应用参数建议
