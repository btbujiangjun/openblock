...
| `s` | RL 状态特征 | 187 | `shared/game_rules.json.featureEncoding` |
| `ψ(a)` | RL 动作特征 | 15 | `features.js` / `features.py` |
| `φ(s,a)` | RL 状态-动作拼接 | 202 | `features.js` / `features.py` |
...
```text
state = 48 维标量（25 结构[含 heightStd + 客观几何 2 维] + 19 颜色摘要 + 4 单步难度） + 64 棋盘占用 + 75 dock 形状 = 187
action = 15 维（block、位置、尺寸、清行、风险、多消、同 icon、清屏等）
phi = state + action = 202
```
