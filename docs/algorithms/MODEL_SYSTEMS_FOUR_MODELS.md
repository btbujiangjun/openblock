...
> **命名规范**：出块相关角色统一使用 `SpawnPolicy*` / `SpawnParam*` 前缀（详见 [`ALGORITHMS_SPAWN.md`（§12）](./ALGORITHMS_SPAWN.md#十二出块算法架构总览工程分层)）；裸名 `SpawnTransformer` / `Spawn Tuning` / `Spawn Generator` 已弃用。
...
| Transformer | Vaswani et al., 2017, [Attention Is All You Need](https://arxiv.org/abs/1706.03762) | SpawnPolicyNet 的条件序列编码与自回归头 |
...
| `s` | RL 状态特征 | 204 (v1.67) | `shared/game_rules.json.featureEncoding` |
| `ψ(a)` | RL 动作特征 | 15 | `features.js` / `features.py` |
| `φ(s,a)` | RL 状态-动作拼接 | 219 (v1.67) | `features.js` / `features.py` |
| `behaviorContext` | Spawn V3.1 行为上下文 | 66 (v1.67) | `spawnModel.js` / `spawn_model/dataset.py` |
| `shape id` | 出块形状词表 | **40**：原 28 + 12 个 2-3 格新形状） | `shared/shapes.json` / `SHAPE_VOCAB` |
...
```text
state = 65 维标量（25 结构[含 heightStd + 客观几何 2 维] + 19 颜色摘要 + 4 单步难度 + 3 空间规划[v1.67] + 3 策略 one-hot + 11 condition token） + 64 棋盘占用 + 75 dock 形状 = 204
action = 15 维（block、位置、尺寸、清行、风险、多消、同 icon、清屏等）
phi = state + action = 219
```

> v1.67 空间规划 3 维（regionEntropy / largestRegionRatio / smallRegionCellRatio，SSOT=`spatialPlanning.js`）拼在单步难度之后、策略 one-hot 之前；condition token 仍占标量末 11 维。填充率只衡量"占多少"，这三维从拓扑/熵视角衡量"占得整不整齐、开放空间是否整片"。
...
| 42 | `scdNorm` | `scd / scdSaturation` | 空间约束密度（三块总格 ÷ 空格），单步几何压力（§14.二 / `spawnStepDifficultyFeatures`） |
| 43 | `comboCellsNorm` | `/ comboCellsNorm(15)` | 本轮候选三块总格数，组合级占用压力 |
| 44 | `comboKillerNorm` | `/ 3` | 致命块数（形状口径：≥killerMinCells 或长条）|
| 45 | `comboLongBarNorm` | `/ 3` | 长条数（单行/列长度≥4）|
| 46–109 | `gridOccupancy[8×8]` | `0/1` | 棋盘空间占用图，CNN / 线性策略都能读到的几何主体 |
| 110–184 | `dockMask[3×5×5]` | `0/1` | 每个候选块居中放入 5×5 mask 后的形状空间编码 |
...
- `_stressBreakdown`：每个压力分量的贡献（v1.71 起追加 `peogActive` / `peogIntensity` / `peogBypass` 三字段）。
- `spawnHints`：出块软目标快照（含 `spawnHints.warmRun` + `spawnHints.peog` 两层钳制元数据）。
- `_spawnDiagnostics`：候选统计、chosen reason、solutionMetrics、reject reason。
- `moveSequence`：把 spawn 与 place 快照写入回放。

#### 4.7.1 机会面（spawnHints）调控机制（v1.71 完整表）

| 机制 | 性质 | 触发 | 与他机制关系 |
|------|------|------|---------------|
| `postPbRelease`（§4.9） | **奖励性减压**：刚破 PB | `postPbReleaseActive=true` | bypass PEOG / challengeBoost / expertEarlyBoost |
| `recovery / nearMiss` | **救济性减压**：玩家陷入困境 | `needsRecovery / hadRecentNearMiss` | bypass PEOG / expertEarlyBoost |
| `farFromPBBoost`（§13.2） | **送爽性减压**：D0 远征段所有玩家 | `raw pct<0.30` | 与 PEOG `min(boost, cap)` |
| `expertEarlyBoost`（§4.15） | **加速性送爽**：高手早期 | `bestScore≥1200 ∧ r_difficulty<0.45` | 与 PEOG `min(floor, cap)`（PEOG 优先） |
| `earlyOvershootGuard`（§4.16 / v1.71） | **保护性压制**：中高 PB 段开局守卫 | `bestScore≥1200 ∧ spawnsUsed<8 ∧ pct<0.85` | 接管温暖局 + 构造算子 + expertEarlyBoost；12 路 bypass |
| `challengeBoost`（§3.4） | **临场加压**：D2/D3 接近 PB | `pct ∈ [0.80, 1.02]` | PEOG `approach_handoff` 时接力 |
| `pbOvershootBoost`（D4） | **持续加压**：超 PB 防膨胀 | `pct > 1.02` | PEOG 自然到期之后才生效 |
| `warmRun.applyWarmRun`（§十七） | **人群保护**：新手/回流/连挫钳制 | T1~T7 触发器 | 在 PEOG 之前应用；PEOG 收紧其释放强度 |
...
| `web/src/spawn/warmRun.js` | 温暖局钳制器（modulator）|
| `web/src/spawn/peog.js` | v1.71 PEOG（PB 早期超越守卫）|
| `shared/game_rules.json` | 配置 profile、阈值、难度、特征维度 |
| `docs/algorithms/ADAPTIVE_SPAWN.md` | 信号矩阵与策略解释 |
| `docs/algorithms/ALGORITHMS_SPAWN.md` | 出块算法总手册（§十七 温暖局 + §17.12 PEOG）|
| `docs/player/BEST_SCORE_CHASE_STRATEGY.md` | PB 主线策略（§4.15 expertEarlyBoost / §4.16 PEOG）|

---

## 五、模型二：生成式出块模型 SpawnPolicyNet
...
| `shape vocab` | **40**） | 可生成的 shape id 数量 |
...
┌──────────────────────────── State Feature Split ─────────────────────────┐
│ s ∈ R^187                                                                │
│   ├─ scalars[48]: fill / holes / near-full / color / step-diff / geo     │
...
图注说明：`s[187]` 被拆为全局标量、8×8 棋盘占用和 3 个 dock 形状 mask；棋盘分支提取空间特征，dock 分支通过交叉注意力读取棋盘上下文；共享表示 `h(s)` 同时服务价值估计、动作打分和辅助监督。

| 模块 | 输入 → 输出 | 结构 | 物理含义 |
|------|-------------|------|----------|
| 标量分支 | `46 → 46` | 直连 | 保留 fill、holes、nearFull、颜色摘要、单步难度等低维全局事实 |
...
| 共享 trunk | `46 + C + 48 → 128` | LayerNorm + 3 层残差 MLP | 得到统一局面表示 `h(s)` |
...
| 参数 | 当前值 | 含义 |
|------|--------|------|
| `stateDim` | 187 | 状态特征长度（含 4 维单步难度 + 2 维客观几何） |
| `actionDim` | 15 | 单个动作特征长度 |
| `phiDim` | 202 | `state + action` 拼接长度 |
...
- [出块建模](./ALGORITHMS_SPAWN.md#十三出块建模双轨实现与设计-rationale)
