...
- [3. 玩家能力画像（PlayerProfile）](#3-玩家能力画像playerprofile)
 - [3.5 stress 域口径](#35-stress-域口径v15517)
...
- [8. 调优指南](#8-调优指南)
- [**10.8 出块算法完整流水线**](#108-出块算法完整流水线v16035-代码基准)
...
- **落地（UI 反馈层，）**：`Grid.getMaxLineFill()` ≥ 0.875（整行/整列差 1 格满）且**体感很差**
...
- **与"无路可走"语义分家**：当 `_handleNoMoves` 触发后会置 `_pendingNoMovesEnd` 互斥锁
 抑制同帧 near-miss toast，并改用独立的 `effect.noMovesEnd`（"棋盘填满，再来一局！"）展示濒死安抚语
 避免同一文案"差一点... 再冲一把！"被复用在三个完全不同的语境里。
- **触发-展示一致性**：toast hold 时长 2.8 s 远长于一次落子，纯靠"触发瞬间几何条件"
...
| `recordPickup()` | 玩家激活候选块（`Game.startDrag` 入口） | 更新 `_pickupAt`，下一次 place/miss 与之相减得 `pickToPlaceMs` |
...
| `metrics.pickToPlaceMs` | ms / null | startDrag → 落子的窗口均值（剔除观察） | **反应**：纯执行段，反映临场操作熟练度与犹豫 |
...
> **背景**：~ 算法层做了大量 stress 精算（lifecycle cap / occupancyDamping / smoothStress / flowPayoffCap / challengeBoost / pbOvershootBoost 等），但用户反馈 "stress 指标不太能体现到玩家感受上来"。审计发现 5 个本可承载 stress 感知的渠道（HUD/视觉/音效/震动/出块）中只有"出块"生效，其余 4 个全部断层。 通过 `stressAmbience.js` 系统补全玩家感知通道。详见 `docs/player/BEST_SCORE_CHASE_STRATEGY.md` §5.α.11。
...
#### stress → 出块算法的 5 条传导路径与 精算细化

> §5.α.11 解决了"玩家显性感知"，但 stress 在算法层的传导仍有台阶感。 对 5 条传导路径做精算细化。详见 `docs/player/BEST_SCORE_CHASE_STRATEGY.md` §5.α.12。
...
| **C. targetSolutionRange 软过滤** | `solutionDifficulty.ranges` 按 stress 选区间，blockSpawn 在三连块通过 sequentiallySolvable 后用 DFS 估算解叶子数软过滤 | ⚠️ P1 新增 '渐紧' 档（minStress=0.5, max=64）填补 0.5~0.6 断档； 在此轴之外新增 **targetHoleIncrement 第二维度**（详见下文）|
| **D. orderRigor 顺序刚性** | `stressTerm` 控制 `orderRigor`，进而决定 `orderMaxValidPerms` ∈ [2, 4] 软过滤 | ⚠️ P0 改用 softplus ramp（smoothness=0.08）消除 0.55 跨阈值台阶；P2 D4 高 stress 强锁死（+0.25 boost） |
| **E. spawnIntent 离散意图** | 6 档枚举（relief/engage/harvest/pressure/flow/maintain），决定 hints 套装与 stressMeter 叙事 | ⚠️ P3 新增 'sprint' 中间档（stress ∈ [0.45, 0.55)）平滑 maintain → pressure 过渡 |
...
#### 第二维度：targetHoleIncrement（空洞强迫度）
...
#### 多轴扩展：9 维 stress→算法 难度投射

> 在 双轴（`targetSolutionRange` × `targetHoleIncrement`）之外，再引入 9 个 O(n²) 廉价度量，把 stress 投射从 2 轴扩展到 **11 轴**。设计动机： 双轴的过滤约束在中段 stress（0.4~0.6）不够锐利，玩家从 D1 跳到 D2 的"压迫感"切换不显著。详见 `docs/player/BEST_SCORE_CHASE_STRATEGY.md` §5.α.14。
...
| 与 共享 stress | 9 维派生器全部使用 `solutionStress`，保证多轴对 stress 单调一致 |
...
| 玩家体验感 | §5.α.11 感官 | §5.α.12 算法 | §5.α.13 算法 | §5.α.14 算法 |
|----------|-----------|----------|-----------|-----------|
...
#### 决策快照增量刷新：消除 DFV / stressMeter 文案与盘面的"快照滞后"
...
#### 决策快照展示层一致性治理：6 项 UI 同源 bug 修复

> 已经把决策层（`spawnIntent` / `spawnDiagnostics.layer1`）做到玩家每次放置后增量刷新，但截图复盘发现 UI 展示侧还有 6 项一致性缺陷分布在 DFV / stressMeter / chip 三处。本节是 的**展示层补完**，从渲染管线、文案分级、视觉降级三个维度收口。
...
`tests/stressMeter.test.js` 两条 / 旧测试被更新到 reason 分级新行为（不再期望返回旧"盘面通透"硬编码文案）。
...
### 3.6 决策派生层（） — UI 消费算法状态的唯一通道

**治理目标**：把 之前散布在 UI 层的"算法 → 显示"转换逻辑统一收口到新的派生层 `web/src/derivation/`，让 UI 永远只读一个 PresentationModel，杜绝 §A/B/D/F/G 类"同一指标多 cache 不同步"的根因。
...
- 未来 计划让 `deriveSpawnIntent` 内部委托 `resolveIntent` 实现单源化

**spawnDiagnostics.layer1 与派生层的衔接**：

- 已让算法层在每次玩家放置后通过 `_refreshIntentSnapshot()` 增量刷新 `spawnDiagnostics.layer1.{fill, holes, nearFullLines, multiClearCandidates, pcSetup}`
- 派生层 `selectLiveGeometry(game)` **优先**读 `layer1`（已是实时值），缺失时降级到 `grid.getFillRatio()`
...
**占用率衰减（occupancyDamping）**——在 clamp 之后、smoothing 之前对正向 stress 乘 `clamp(boardFill / 0.5, 0.4, 1.0)`。低占用盘面（如 fill=0.39）的伪高压由 0.89 → ~0.69（进入 `tense` 而非 `intense`）；fill ≥ 0.5 时无衰减；负向 stress（救济）不被衰减。该项作为单独信号写入 `_stressBreakdown.occupancyDamping`，并在 `stressBreakdown.afterOccupancy` 上记录衰减后的中间值。
...
**spawnIntent 单一对外口径**——除了多档 `shapeWeights` 与连续型 `spawnHints`，引擎还输出离散 `spawnHints.spawnIntent ∈ { relief, engage, pressure, flow, harvest, maintain }`。所有"意图描述"（拟人化压力表叙事、商业化策略文案、回放标签、推送文案）必须读这一字段，不再各自从信号里推断。优先级：
...
3. **`harvest`** — * 收紧*：`nearFullLines ≥ 2` 或 `(pcSetup ≥ 1 && fill ≥ PC_SETUP_MIN_FILL=0.45)`（避免低占用盘面误触发"密集消行机会"叙事）
4. **`pressure`** — `challengeBoost > 0` 或（`challenge_payoff` 且 `stress ≥ 0.55`）
5. **`flow`** — `flow_payoff` 或节奏 `payoff`
6. **`maintain`** — 默认中性

**AFK 召回（engage 路径）**——传统做法是「降难度+小块」让玩家喘息，但实际效果常常是连续给出 4 个单格 + 1×3 横条，盘面瞬间清爽，玩家依然提不起兴趣。新设计在 `profile.metrics.afkCount ≥ 1` 且 `stress < 0.55`、未触发挫败/恢复/新手保护时启用：`clearGuarantee≥2 / multiClearBonus≥0.6 / multiLineTarget≥1 / diversityBoost≥0.15`，rhythmPhase 由 `neutral` 切到 `payoff`，给玩家"显著正反馈 + 可见目标"（一根长条 + 多消机会）。

**rhythmPhase = 'payoff' 几何兜底**——之前所有"基于玩家状态"的分支（`delight.mode='challenge_payoff'/'flow_payoff'`、`playstyle='multi_clear'`、`afkEngage`、`pcSetup ≥ 1`）会无条件把 `rhythmPhase` 升到 `payoff`，于是 17% 散布盘面也会出现：UI pill 「节奏 收获」+ 出块偏向 1×4 长条 + strategyAdvisor 弹「收获期」+ stressMeter 报「密集消行机会」——盘面其实没有任何近满行。新增统一 helper：
...
**clearGuarantee 物理可行性兜底**——`cg=3`（来自 warmup `wb=1` 或 `roundsSinceClear ≥ 4`）含义是"本轮强制 ≥3 块能立刻消行"。但若 `multiClearCandidates < 2 && nearFullLines < 2`，盘面物理上无法兑现，UI pill「目标保消 3」即变成空头支票。最终兜底：在所有 `cg` 调整之后，若 `cg ≥ 3` 且无任何几何支撑则回钳到 `2`，仍保持友好出块语义。

**叙事颗粒度补丁**——一致性补丁解决了"系统说一套做一套"，本节再处理"做对了但还能讲得更准"。共 5 处：
...
**AdaptiveSpawn live 几何覆盖**—— 已把 panel/策略卡的多消候选改成 dock 优先，但 adaptiveSpawn 内部仍主要读 `ctx.nearFullLines/multiClearCandidates`（上轮快照），存在时序偏差窗口。本节把 spawn 决策入口同步到 live 几何。
...
**flow 叙事相位变体表**—— 修了 stress story 优先级倒置，本节再修 `SPAWN_INTENT_NARRATIVE.flow` 文案与实际 rhythmPhase 硬冲突。
...
1. **`stressMeter.SPAWN_INTENT_NARRATIVE.flow` 拆按 rhythmPhase 选变体**
 ...
 因 的 `nearGeom` mutex 会 fall through 到 `'setup'`。结果三方叙事对立：
 - story："心流稳定，**节奏进入收获期**…"
...
**叙事优先级 + 收获期 live 几何 mutex**—— 修了卡间互斥与 sparkline tooltip 解读，本节再修 stress story 文案优先级倒置 与「收获期」卡漏掉 live 几何 mutex 两处残余冲突。

1. **`stressMeter.buildStoryLine`：spawnIntent 永远优先**
 引入 spawnIntent 优先级时为防止"系统真在保活时硬信号被吞"加了 gating
 `frust > -0.08 && recovery > -0.08`，但 后 stressMeter label/vibe 已经诚实化
...
2. **`strategyAdvisor`「💎 收获期」卡加 live 几何 mutex + 待兑现变体**
 ...
 多消候选 0 + 近满 0，dock 是 4 块 volleyball L 形根本消不了任何行。 已为
 「多消机会 / 逐条清理 / 瓶颈块」3 张卡都加了 live 几何 mutex（ 通过 panel 把
...
**卡互斥 Build vs Harvest + Sparkline Help 解读**—— 修了 phase 撞墙与 borderline 翻面，本节再修策略卡间叙事拉扯，并把 sparkline tooltip 从"指标定义"升级为"如何读图"。

1. **`strategyAdvisor`「规划堆叠」加 `harvestNow` 互斥**
 已为「提升挑战」卡加了 `harvestNow = (rhythmPhase==='payoff' || spawnIntent==='harvest')`
...
3. **「挑战」(challengeBoost) tooltip 显式说明触发条件**
 ...
 （adaptiveSpawn.js:615-622 + 5 条单测）已健壮，本次只补叙事层。

**Phase Coherence + Snapshot Marker + Borderline 去抖**—— 修了一帧三方 F(t)/feedbackBias/flowState 撞墙，本节解决残留的 phase 撞墙 + UI 视觉混淆 + borderline 翻面。

1. **`deriveRhythmPhase`：`'setup'` 与 `'harvest'` 互斥兜底**
 加 `canPromoteToPayoff` 时只堵了 `'neutral'→'payoff'`，没堵 `'setup'`
...
**末段崩盘 stress 失真修复**——screenshot 实测发现高分濒死场景下 `stress=0.04` 显示舒缓档、`flowState=bored`、`spawnIntent=harvest`，与玩家 `momentum=-0.53 / 最后 8 步 0 消行` 真实状态严重错位。根因：依赖累计均值的 metric 被前 5 分钟良好表现稀释。本节是该问题的全栈修复。
...
6. **`stressMeter` 挣扎中变体**（`getStressDisplay` 加 `distress` 入参）
 ...
 系统已强制 relief 出块抢救节奏"。优先级高于 的 relief 救济变体。
...
**Live ↔ Snapshot 一致性补丁 + 标签解耦**——~ 解决了"指标内涵不准"，本节再处理"同一帧不同来源说不同话"。
...
1. **`strategyAdvisor`：多消机会卡 / 瓶颈块卡读 live 几何（替代 spawn 快照）**
 ...
 让 `playerInsightPanel._render` 把 `liveTopology` 和
...
3. **sparkline `pacingAdjust` 标签 「节奏」→「松紧」**
 已把 `pacingPhase` UI 标签解耦为「Session 张弛」，但 sparkline 仍叫
...
**multiClearBonus 几何兜底 + 救济 pill 自动化**—— 把所有"做对了"的事都讲清楚了，本节再补两处仍能撞墙的地方。

1. **`adaptiveSpawn`：`multiClearBonus` / `multiLineTarget` 几何兜底**
 与 cg 兜底同源。当 ① `multiClearCandidates < 1` ② `nearFullLines < 2`
...
2. **`playerInsightPanel`：救济 pill 自动化为 top-N 负贡献**
 三件套（frustrationRelief/recoveryAdjust/nearMissAdjust）只覆盖 3 条
...
**调制表（`lifecycle/lifecycleStressCapMap.js`，全仓 single source of truth； 补全为 25 格）**：

> 格式：`cap / adj`。cap = stress 硬上限；adj = 整体偏移（负=减压，正=加压）。
> 完整 SkillScore 计算公式、三条接入路径（cap/adjust + onboarding 覆写 + winback 包）
> 的详细推导见 [§10.10.10 生命周期 × 成熟度如何作用于出块算法](#101010-生命周期--成熟度如何作用于出块算法)。
...
> 之前，S0·M1+、S2·M4、S3·M0 等 8 个组合缺失（`getLifecycleStressCap` 返回 `null`
> raw stress 直通形成"死键"）； 按行内单调递增、列内 S0/S4 弱 / S2/S3 强原则补全为 25 格。

**两个维度的影响幅度（实测）**：

- **stage 固定，band 移动**：同阶段内 M0→M4 → cap 抬升 0.13–0.23 → 对应 10 档 difficulty profile 的 **2–4 档**差距；
- **band 固定，stage 移动**：S0/S4（保护期）vs S2/S3（挑战期）同 band 下 cap 差距 0.10–0.20。
...
> 历史备注：.x 之前 `web/src/retention/difficultyAdapter.js` 定义了一套基于 maturity L1–L4 的 `MATURITY_DIFFICULTY_ADJUST = { L1: stressOffset:-15, L2: -5, L3: 0, L4: 5 }` 平行实现，但全仓**没有任何生产代码调用它到 spawn 路径**（仅自测引用），是 v1 时期遗留。.x 已**移除**该模块，统一由本文小节描述的 `(stage·band) → cap/adjust` 调制表接管。
...
| `multiLineTarget` | 0~2 | 显式偏好「同时多线」兑现 | 阶段 1 排序与 multi 选取；`multiClear≥2` 额外乘子 |
...
| `scoreMilestone` | bool | 刚跨**局内分数里程碑**（区别于 `maturity_milestone_complete` 的跨局成熟度晋升） | 三条通路：①`adaptiveSpawn` 内部抬高 `clearGuarantee` / 压低 `sizePreference`（出块友好化）；②`game.js spawnBlocks()` 顶部桥接到 `_spawnContext.scoreMilestone`，触发 `blockSpawn.js:870-872` 的 `*= 1.3` 加权；③策略卡 / Player Insight 面板 / DFV 仍展示数据流（**UI float toast 已于 撤销**） |
...
| `targetHoleIncrement` | min/max 或 null | **新增**：新空洞难度档位（与 targetSolutionRange 并列双轴）| 通过 DFS 叶子 isolated-holes delta 软过滤候选 triplet——低 stress max 强约束（必有干净解）/ 高 stress min 强约束（玩家必须吞洞） |
| `targetMaxHoleIncrement` / `targetHoleIncrementGap` / `targetEndFillRatio` / `targetNearFullDelta` / `targetFirstMoveSurvivorRatio` / `targetSolutionDiversity` / `targetEndFlatness` / `targetEndDangerColumns` / `targetVisualClutter` | min/max 或 null | **新增**：9 维 stress→算法 难度投射（详见 §3.5 节）| 9 个独立潜意识压力源——专注度税上界/差距、空间窒息、消行节律、试错代价、解多样性、凹凸审美、爆顶预警、视觉杂乱 |
| `orderRigor` | 0~1 | **新增**：顺序刚性强度（0=不约束，1=必须按特定顺序） | 仅用于诊断/面板展示 |
| `orderMaxValidPerms` | 1~6 | **新增**：硬上限 — 6 种排列里允许的最大可解数 | `blockSpawn` 在早期 attempt 拒绝 `validPerms > N` 的 triplet |
| `farFromPBBoostActive` | bool | **§2.1 新增**：远征送爽是否激活（pct &lt; 0.30 且无 bypass 时为 true） | 触发 §2.5 `blockSpawn` 端 multiClear≥2 块权重 ×1.15；触发 §4.3 stressMeter "远征段送爽中" 叙事；豁免 multiClearBonus 几何兜底（≤0.4），让远征段空盘开局仍能保留 multiClearBonus floor=0.45 注入 |
| `winbackProtectionActive` | bool | ≥7 天回流玩家保护期标识 | UI / 商业化 / 推送可据此判断"前 3 局保护期内" |
...
**之前**两者都被叫"milestone"且文档里没有显式区分；现统一以 `scoreMilestone` vs `maturityMilestone`
...
- **多消概率**：`playstyle='multi_clear' / 'combo'`、`comboChain` 活跃或高消行效率玩家，提高 `multiClearBonus` 与 `multiLineTarget`；无几何支撑时仍受 软封顶。
...
| §6 `scoreMilestone` | §2.5.5 修复历史：`spawnHints → _spawnContext` 桥接，`blockSpawn.js:870-872` 从 dead branch 变为真生效 |

---

## 10.6 外部实证基线：SGAZ × Tetris Block Puzzle（）
...
| 形状库 | 标准 tetromino（+ 可选 U/V/X/T-pentomino） | **40 个变体**：超小直线 + 标准 tetromino + 内部变体 + 斜线散点 + 3 格 L 角；**无 pentomino**（5 格只保留 1x5/5x1/l5-*） |
...
| **形状库扩充** | **★★ 强** | 加任意一个 pentomino 即可让奖励显著下行；加两个 pentomino 在 `h=2, p=0` 下直接训练不收敛；**T-pentomino 单独造成最大减速** | ✓ ：28 → 40 形状（+ 极小直线 / 斜线散点 / 3 格 L 角），并按"加减压策略"做 gate + 加权（详见 §10.7） |
...
- D2 阈值 0.80 / D3 阈值 0.95： 即用阈值，保持与 `challengeBoost` / `bestScoreMilestoneCheck` 兼容
...
`adaptiveSpawn.js:1745` 的"几何缺失时把 multiClearBonus 封顶到 0.4"逻辑， 把 `farFromPBBoostActive` 加入豁免名单（与 `afkEngageActive` 同类）。理由：远征段开局通常恰好命中"_mcCands<1 && _nfLines<2 && !_realPcSetup"的空盘特征，若不豁免则 floor=0.45 注入会被立即撤回，让送爽机制失效。
...
- `orderRigor` 公式：`max(0, stress-threshold)*scale + max(0, skill-0.5)*0.20 + difficultyTuning.orderRigorBoost + motivationBoost`
...
`web/src/stressMeter.js buildStoryLine()` 在 `boardRisk >= 0.6`（保活）之后、`shouldUseScorePushHighStress`之前插入 PB 距离段抢占：
...
`tests/bestScoreChaseStrategy.test.js` §5.α 段（22 个用例）：
...
| §4.3 | stressMeter PB 联动叙事 + 高 boardRisk 优先级 | 3 |
 ...
 §5.α.6 段（12 个用例）见下方 §13.7。

### 13.7 认知一致性守卫：`pbChase.minBestScoreForIntenseFeedback`（）
...
**`challengeBoost` 不受守卫影响**（设计取舍）：
 ...
 已有的 `challengeBoost` 机制已经 cap 在 0.15 内且有 5 路 bypass，低水位玩家在 D2/D3 段仍然能感受到"逼近最佳"的轻度加压。本守卫只 bypass 新增的两个机制，避免与 已稳定的路径互相干扰。
...
- `challengeBoost`：D2/D3/D4 段数值加压（cap 0.15）
...
### 13.9 三原则下的算法完整闭环：D4 持续加压 + D0 分级减压 + PB 增长率反向加压（）

 §5.α.7 确立"策略隐性"原则后，算法层在三大原则下还存在 3 处关键缺口（详见 [BEST_SCORE_CHASE_STRATEGY §5.α.8](../player/BEST_SCORE_CHASE_STRATEGY.md#5α8-三原则下的算法完整闭环d4-持续加压--d0-分级减压--pb-增长率反向加压v1564)）。本节聚焦算法侧实现细节。
...
#### 13.9.2 D4 段 `pbExtremeOrderBoost` 延续到 D4（弱强度）
 ...
 原版：`score < ctx.bestScore` 硬条件，破 PB 后立即关闭。
 改为：`_commonOrderGates` 公共门 + 分支
...
#### 13.9.7 同源 bypass 链（与 §5.α.6 一致）

所有 新增机制全部受以下 bypass 约束：
...
### 13.10 D4 加压链路 4 处冲突完整修复（）

 §5.α.8 在算法**逻辑层**增加了 pbOvershootBoost / D4 spawnHints 收紧，但**端到端 stress 链路审计**发现 D4 加压被 4 处机制反向消解。本节聚焦修复细节。
...
### 10.7 形状池扩展（→ ：独立库事件注入系统）

> **留存数据驱动的平台分化**：Android/微信小程序档 `MONO_FLUSH_PICK_PROBABILITY`
...
§10.6 论文实证「**形状库扩充**是 ★★ 强级杠杆」——OpenBlock 在 之前从未动用，仅靠 `shapeWeights` profile 插值（★ 中级杠杆）。**** 首次以"形状库扩充"作为减压/加压双向手段，**** 将其重构为完整的"独立库事件注入系统"，**** 进一步把"触发"从纯几何启发式升级为**阶段绑定 + 三类触发分级**：
...
- **关键差异**：旧版 relief 仅看几何信号（`hasClearSetup ‖ highFillFillHoles ‖ monoFlush`），任意意图都可触发——导致 harvest/maintain 等中性意图下也会塞 1×2/l3-* "减压块"，与 chosen 主路径的消行候选语义冲突。新版要求 `isReliefPhase = true` 作为 hard gate，从根本上消除"意图—注入语义对掐"。

- **可控**：每局上限 = `max(totalClears × 10%, 3)`、间隔 ≥ 5 轮、relief/pressure 各自子配额
- **可回放、可观测、可豁免空洞**： 全面契约化（详见下文 §架构 + §三大契约）；
 新增 `reliefTrigger` audit 字段（pcSetup/exactFit/multiClear/monoFlush 四枚举），DFV / spawnDiagnostics 可按触发类型聚合统计
...
```
 ...
 ▼
 ┌──────────────────────────┐
 │ Fisher-Yates(triplet, │ 接受 ctx.rng
...
3. 默认行为不变：未传 rng 时降级到 `Math.random`，与 行为一致
...
> 同时增强了 `scripts/sync-core.sh`：新增 `lib/` 子目录分发支持（与 `bot/` 同级），未来 web 端引入任何 `lib/` 工具都不需要再改脚本。

---

### 10.7.1 —— 缺口闭环（系统性自查 5 项修复）

 重构后，10.7 末尾自查列出了 5 个未闭环缺口； 一次性补齐。
...
| **#5 bot 仍看 coverable** | ⚠️ `_tryInjectSpecial` 读 `topo.holes`，与 UI 玩家心智口径错位 | 信号侧改读 `topo.enclosedVoidCells`，降级到 `topo.holes` 保证后兼容。`highFillFillHoles` 与 `notAlreadyFullOfHoles` 两个条件都受益 —— bot 现在跟玩家"看到"的洞数同口径 | `_tryInjectSpecial` Step 1 `holesSignal` 推导 |
...
`tests/blockSpawn.test.js` `describe(' 独立库注入系统 — 缺口修复')`：
...
9. （三道 gate 不破坏 既有 7 项不变量，34 个原有 blockSpawn 测试全过）

全量回归 **1907 / 1907**。

---

### 10.7.2 —— 三层防御（"空盘出 special-relief" 兜底）

#### 触发根因

 把信号侧切到 `enclosedVoidCells` 后，发现一个边界场景：截图显示 `fill ≈ 0.05` 的近空盘上仍出现了 `L3-` 减压块，DFV chosen 标 "因·特殊减压"。
...
| # | bug | 表现 |
|---|---|---|
| **A** | `chosenMeta.push` 三处都没传 `pcPotential / multiClear / gapFills` 字段 | Step 4 智能 replaceIdx 评分公式 `pcPotential*4 + multiClear*2 + gapFills + placements/50` **实际全部为 0**，等价于按 placements 排序——清盘候选未受任何保护 |
...
| **C** | `diagnostics.chosen` 序列化时只保留 `id/category/reason/topDriver`，**裁掉**所有评分字段 + audit 字段（`original / originalMeta / injectedAt / subType / spawnCtx`） | DFV ⚡ badge tooltip 无法展开"原 X → 替换为 Y"，可解释性面板拿不到 pcPotential 用于策略说明 |
...
// Step 4（ 增强）：pcPotential >= 2 槽位不可替换
...
### 10.7.4 —— 多步可达清盘保护 + DFV spawnCtx 解释

#### 用户定义的"清盘逻辑"严格语义

> 放置候选块、**或者按特定次序放置多个候选方块后**，可以实现盘面全消、清空。

 的 Step 1.8 / Step 4 仅保护**单步可达**（chosen 含 `pcPotential === 2`）。多步可达（"3 块按某序放完后清空"）未覆盖——若 chosen 中无任一块单步能清盘但 3 块组合存在某顺序能清盘，relief 注入仍会抢走清盘机会。
...
之前 ⚡ tooltip 仅显示 `"⚡事件注入：原 X → 替换为 Y（减压信号触发，slot #N）"`。 新增**触发上下文行**：
...
| 字段 | 对照防御层 | 玩家可见效果 |
|---|---|---|
| `fill` | Step 1.7 (≥ 0.25/0.10) | 看到 fill 才知道为什么 0.05 空盘场景不会注入 |
| `pcSetup` | Step 1.85 (≥ 1 才查多步) | 看到 pcSetup=0 才知道没走多步 PC 检查 |
| `holesSignal` | enclosedVoidCells | 与 UI "Holes" 同口径，无错位 |
...
### 10.7.5 —— exactFit 完美卡入策略（"2×2 凹槽 + 2×2 候选块"场景识别）
...
### 10.7.6 —— monoFlush 同花顺消除策略（×5 倍 iconBonus 算法识别）
...
DFV chosen tooltip 释义：「bestMonoFlushPotential 真模拟，本块可补满 N 条"已填部分同 icon"的行/列，染色匹配后触发 ×5 倍 iconBonus 得分（同花顺大消除）」
...
### 10.7.7 —— 完美卡入优先级反转（exactFit=1.0 > multiClear）

#### 用户反馈与诊断

> 截图中候选块都标"送消行 可消N行"，没有体现 exactFit；**优先适配完美卡入**。

 把 exactFit 排在 multiClear 之后是"保守设计"——理由是"消行是真模拟硬 payoff，几何嵌入不一定消行"。但实践中发现：
...
| **紧凑卡入**（exactFit ∈ [0.85, 1.0)）非确定性闭合 | 保留 在 multiClear 之后的位置（让位给真模拟消行） |
...
将原 的"multiClear>=1 优于 exactFit"断言改为**分场景反转**：

- 紧凑卡入 0.875 + multiClear=1 → 走 multiClear（原行为不变）
- 完美卡入 1.0 + multiClear=1 → 走 exactFit
...
### 10.7.8 —— 高/极度 novelty 场景"双胞胎/三胞胎"重复块注入
...
`validateSpawnTriplet` 中硬约束 `duplicate-shape` —— 之前 chosen 永远不允许出现重复。3 个阶段（`clearCandidates` / `weighted` / `fallback`）都用 `usedIds` 去重。
...
### 10.7.9 —— monoFlush 与染色 bias 同口径修复
...
| chosen 评分 | `Grid.bestMonoFlushPotential` | "row 上非 shape 部分全填同色" — **无 empty 门控** |

→ 两个函数**口径不一致**：
- shape 占 5-6 格 + 已填 4-3 格同色 → 误计 `monoFlush=1`
...
由于 monoFlush 门控收紧，原本因 bug 误标"可凑1同花顺"的 chosen，现在会回落到：

- **真正满足 empty ≤ 2** → 仍标"可凑1同花顺"（valid）
- **不满足 empty ≤ 2** → 走 multiClear/exactFit/gapFills 等下一级 driver

这是**正确的回退路径**：原本就该走的 driver，被 bug 抢占了。修复后 DFV 解释与真实算法行为完全一致。

#### 不变式测试（`tests/monoFlush.test.js` 子套件）
...
### 10.7.10 —— monoFlush 触发的"事件注入"路径补全

#### 用户反馈与盲区定位

 修复了 `bestMonoFlushPotential` 的口径，但同一天截图复现仍发现：
...
- **DRIVER_SEMANTIC[monoFlush]** 与 一致复用（同口径）
...
**`tests/monoFlush.test.js` 子套件（8 个）**：
...
**`tests/blockSpawn.test.js` 子套件（7 个）**：
...
| 版本 | 作用域 | 解决问题 |
|---|---|---|
| | `scoreShape` + `bestMonoFlushPotential` + DFV | 主路径识别 monoFlush，普通块也能"凑同花顺" |
| | `bestMonoFlushPotential` 阈值 | 修复 `empty` 计数 caliber，与染色 bias 双向锁定 |
| **** | `_tryInjectSpecial` + `findNearFullMonoLines` + DFV | **独立池**作为兜底——主路径不命中时（如截图 col 8 填 hook 的"必须靠 1×2/2×1"场景），事件注入路径接手 |
...
### 10.7.11 —— monoFlush 主路径直通：放行特殊形状 + 消行 seat 选拔
...
新截图 chosen 仍为 `5×1 / L3 / 4×1` 普通块 + multiClear/gapFills/shapeWeight driver——**的"独立池兜底"路径完全没生效**。
...
3. 同时主路径 `scored` 列表又没有 1×2/2×1（被 `_passesShapeGate` 一刀切拒绝）， 的 `scoreShape × monoFlush` 加权**根本无形状可加权**；
4. 形成"两层都未命中"悖论：主路径**看不到**形状，注入路径**被 gate 拒**。

#### 修复设计：1×2/2×1 在 monoFlush 信号下直通主路径

**核心思想转变**：从"独立池事件注入"改为"**条件性放行**进入主路径"——
- 的 `scoreShape × monoFlush × iconBonusTarget` 加权直接命中；
...
**之前的致命 bug**：`clearCandidates` 过滤条件**不包含 monoFlush**：
...
#### 与 的关系（双层 fallback 架构）

| 层 | 路径 | 实现位置 | 命中条件 | 实际游戏中触发率 |
|---|---|---|---|---|
| **L1 主路径** | `generateDockShapes` 主流程 | `scored` → `clearCandidates` → pick | monoFlushAllowIds 非空 + 复校通过 | **高**（截图复现场景命中率 ≥50%） |
| **L2 注入兜底** | `_tryInjectSpecial` monoFlush 路径 | 8 道 gate 全通过 | L1 未命中 + warmup/fill/PC/节流/配额全通过 | **极低**（实际游戏几乎不触发） |
...
| 无 monoFlush 信号 | `monoFlushAllowIds=空` → 1×2/2×1 仍被 `_passesShapeGate` 拒 | 同 |

#### 不变式测试（`tests/blockSpawn.test.js` 子套件）
...
4. **无信号场景**：散点杂色盘面 → 50 次抽样 1×2/2×1 泄漏 ≤ 5 次
...
### 10.7.12 —— monoFlush 建设期信号补全（empty ≥ 3 场景识别）
...
~24 的 `bestMonoFlushPotential` 严格要求 **某 row/col 上 `empty ∈ [1, 2]`**（与 `monoNearFullLineColorWeights` 染色 bias 双向锁定）。在新截图场景：

- 大片 5×5 同 icon 已成型，但任一 row/col 上仍 `empty ≥ 5`（远未近满）；
- `bestMonoFlushPotential = 0` → driver 不触发；
- 的 `findNearFullMonoLines` 也返回空 → 主路径放行机制不激活；
- 的 `_tryInjectSpecial` monoFlush 路径同样不触发。

**结论**：~24 的整个 monoFlush 识别体系**只覆盖"兑现期"（即将补满）**，对"**建设期**（朝 8 同色累积）"完全无信号——这正是用户语义中"同花顺机会"的更广涵盖范围。
...
- DFV label `建N同花` 与 `可凑N同花顺`视觉/语义上区分清晰。
...
- `DRIVER_SEMANTIC.monoFlushBuildup` = `"bestMonoFlushBuildup 真模拟，本块在 same-icon line 上贡献 N cells，朝 8 同色累积（建设期，与 monoFlush 兑现期互补；本块放下不立即触发 ×5，但为后续 monoFlush 创造条件）"`
...
盘面有近满同色 line？
 ...
 │
 └─ 否 → 主路径常规 scoreShape 选拔（含 的 monoFlush=0 时不加权）
 → 1×2/2×1 仍被 _passesShapeGate 拒绝，维持独立池语义
```

---

### 10.7.13 —— monoFlush 严格定义重构（撤销 阈值 + 撤销 buildup driver label）

#### 问题复现

 实施后第三天用户再反馈，并首次提供**严格定义**：
...
`bestMonoFlushPotential` 引入 `NEAR_FULL_MIN_PREFILLED = n - 2 = 8`，意图"与染色 bias 同口径"，但实际效果等价于 **shape 只能占该 line ≤ 2 cells**。
...
**Bug 1（中间块误标"建2同花"）—— buildup 不该作 driver**：

 的 `monoFlushBuildup` 只要求"shape 贡献给 same-icon line ≥ 6 cells 累计"，**不要求消行**。按用户严格定义，shape 放下**不消行**则**不构成同花**。driver label "建N同花" 让玩家误以为有 ×5 iconBonus。误报。
...
- 更新 `monoFlush` 语义文案为 严格定义："本块放下后会触发：消行（line 满）+ 全 line 同 icon —— 即立即触发 ×5 倍 iconBonus"
...
**辅路保留**（不变）：
- `_tryInjectSpecial monoFlush 路径`（L2 注入兜底，8 道 gate 后注入 1×2/2×1）
- `1×2/2×1 主路径绕过 _passesShapeGate`（L1 直通）

全量回归 **2032 / 2032**。

---

### 10.7.14 续 —— "送清屏"badge 严格按 shape 自身 pcPotential 派生
...
#### 不变式测试（`tests/blockSpawn.test.js` 续子套件）
...
`_estimateTopDriver` 的优先级 `pcPotential > monoFlush > exactFit > multiClear > gapFills`与 reason 派生独立——driver 仅是 chosen 块 label，reason 是 spawn 来源 badge：
...
### 10.7.15 —— monoFlush 染色强制绑定（修复"driver 标可凑同花顺但实际不染对应色"漏标）

#### 问题复现

 续 修复"送清屏"badge 后，用户再反馈：

> 同花顺判断逻辑仍然不对

截图：chosen 第三块 driver `"可凑1同花顺"`（`monoFlush>=1`）+ badge `"送清屏"`（已被 续 修复，疑似截图前未刷新），但**核心问题**是 driver 标"可凑同花顺"经过多次修复后仍存在"虚标"——shape 算法层面是同花潜力，但实际玩家放下大概率不触发 ×5 iconBonus。
...
### 10.7.16 —— monoFlush 降为"乐趣彩蛋"（25% 概率门槛节流）

#### 设计动机

~27 经多轮修复后，monoFlush 信号体系已完整：
- **判定准确**
- **染色绑定**
...
判定与绑定都对了，问题在于**命中率**——之前同花顺信号下 chosen 几乎 100% 含 monoFlush shape，driver 必标 "可凑N同花顺"，**常态化反而失去惊喜彩蛋语义**，且遮蔽其他乐趣（multiClear / pcPotential / exactFit）。
...
更新 3 条 测试：1×2/2×1 命中预期从 `≥10/20 (50%)` 调整为 `≥2/20 (10%)`，反映 节流。

#### 完整 monoFlush 闭环（→28 累积契约）

```
generateDockShapes 入口：
 ↓
monoFlushRound = Math.random() < 0.25 ← 源头 gate
 ↓
┌──────────────────────────────────────────────────────────┐
...
│ 6. game.js 染色绑定 dockColors[i] = targetCi（）│
...
### 10.7.17 —— 特殊块标题强化 + 单 dock 同花顺数量限制 + 染色多样性
...
**问题**： 加 25% 轮次门槛后，命中轮次内仍可能 chosen 三槽**全部**标"送同花"——bias 在 monoNearFullLineColorWeights 集中向单一颜色 + scored 中多 shape 都能 monoFlush → 三槽同色同 driver → 视觉过载且彩蛋稀缺感丧失。
...
**Invariant**（`tests/blockSpawn.test.js` 子套件）：60 轮强 monoFlush 场景中**单 dock monoFlush 最大数 ≤ 1**。

#### 改造 3：dock 3 块严格无放回染色（同色仅作彩蛋锁定色）

**问题**： 染色绑定 + bonusBias 让多槽抽到同色：
...
### 10.7.18 —— 自适应概率 + 识别 always-on（修复 强信号漏识别 bug）
...
#### 不变式测试（`tests/blockSpawn.test.js` 子套件，新增 3 条）
...
### 10.7.19 —— 极小概率惊喜 + 修复 计数泄漏（用户截图反馈"同花概率过高"）
...
| **#2 频率过高** | "经常出现" | 自适应概率把强信号场景推到 65-90% cap，违反"极小概率惊喜"诉求 |
...
- 修复 早期 bug：1×4 既是 pcPotential 又是 monoFlush 候选，被过滤后导致 chosen 漏失清屏机会（`prioritizes direct perfect-clear candidates` 测试失败）
...
### 10.7.20 —— 同花降 2/3 + 大幅提升清屏（用户反馈"概率重平衡"）
...
#### 不变式测试（`tests/blockSpawn.test.js` 子套件，新增 2 条）

1. **同花强信号命中率 ≤ 20%**（200 轮， 的 ≤45% 的 2/3）—— 验证降频
...
```
Test Files 110 passed
Tests 2051 passed ( → 净增 2)
blockSpawn 89/89 × 3 次连跑稳定
```

---

### 10.7.21 —— 预算守卫双修：avail 兜底误标 + topDriver 上下行矛盾
...
```
Test Files 110 passed
Tests 2051 passed（ → 零净增，纯 bug 修复）
...
| G6 | **兜底硬抑制**：`isRelief && reliefTrigger ≠ 'monoFlush' && chosen.multiClear≥1 计数 ≥ 2` → 不注入（pcSetup/exactFit trigger 下也覆盖 R11 截图根因） |
...
> 旧版 – 的"几何驱动"分支（无 intent + 高填充 + holes>5 → relief）已废弃；
> "裸 pressureSignal without intent → pressure" 兜底分支同样移除。

**monoFlush 触发 relief 路径"子触发但不受压制）**：
...
- 候选命中需通过 `bestMonoFlushPotential` 真模拟才标 `reason='special-monoFlush'`
...
> **基准**：代码版本 ；棋盘固定 `gridWidth = 8`（`game_rules.json`），总格数 64。
...
| L25 | `multiClearBonus` | 多消鼓励强度 [0,1] | `max(deriveMultiClearBonus(ctx, fill), delight.multiClearBoost)`；分段规则：pcSetup≥2→1.0；pcSetup=1→0.9；nearFullLines≥5→1.0；≥3→0.8；roundsSinceClear>3→0.7；fill>0.60→0.6；fill>0.45→0.4；其他→0.22；Android/微信底值≥0.15 |
...
| # | 意图值 | 触发条件 | 后续效果 |
|---|---|---|---|
| L43 | `'relief'` | playerDistress < −0.10 ∨ delightMode='relief' ∨ forceReliefIntent=true ∨ **delightStarved=true** | clearGuarantee≥2，sizePreference→负，偏小且能消行的块 |
