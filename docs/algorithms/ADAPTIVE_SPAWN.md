...
| 层 3 | 优先选拔层 | §10.8.4 | 候选筛选 → 动态席位（1–3 席）→ 清屏 > 同花 > 消行 > 随机分支选取；单轮同花 ≤ 1 预算守卫 |
| 层 4 | 加权补齐层 | §10.8.5 | 14 维乘法权重链（机动性 · 清屏 · 多消 · 精确卡入 · 同花 · 空洞修复 · 节奏 · 品类多样 · PB 段位…） |
| 层 5 | 约束验证层 | §10.8.8 | 最低机动性 · 顺序可解性（fill ≥ 52%）· 解法空间 · 九维区间；最多 22 次重试 |
