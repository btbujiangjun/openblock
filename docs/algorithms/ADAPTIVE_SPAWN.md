...
**调制表（`lifecycle/lifecycleStressCapMap.js`，全仓 single source of truth；v1.55 补全为 25 格）**：
...
> v1.55 之前，S0·M1+、S2·M4、S3·M0 等 8 个组合缺失（`getLifecycleStressCap` 返回 `null`，
> raw stress 直通形成"死键"）；v1.55 按行内单调递增、列内 S0/S4 弱 / S2/S3 强原则补全为 25 格。
