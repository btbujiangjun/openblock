...
sample 必须真实反映 (ctx, θ) 配置下启发式算法 + bot 跑出来的 d_curve,**不能把 ideal target 数据泄露进 sample**。否则 model 学到的是数学公式而非真实算法的行为,θ 优化器找的"最优 θ"对真实部署无意义。
...
接受这个客观事实:**8×8 grid + 启发式规则在物理上达不到 ideal 0.10 → 1.00 的端点**:
...
### 3.3.1 事实门禁 `fact_eval`——以目标 S 曲线为准判定「效果是否提升」
...
| Loss | 权重 | 拉力方向 | 业务作用 |
|---|---|---|---|
| `L_target_fit` (主) | **5.0** | model → ★ **ideal** target_S_curve (跨度 0.90) | ★ 直拉业务目标 |
...
overrides = {
    0: 3.0, 1: 2.5, 2: 2.0, 3: 1.5,     # ★ 低 r 强化
    7: 2.0, 8: 3.0, 9: 4.0, 10: 4.0, 11: 3.0, 12: 2.5,   # 拐点
    15: 2.0,                             # 高超 PB
}
```

注:`bin_idx = int(r / bin_width + 1e-9)`(修复浮点 bug,例 `int(0.6/0.1)` 原本得 5 应得 6)。
...
|------|-------------|-----------|------|
| 当前实现 | 每 ctx 跑 surrogate 优化得到 best θ* | `model-inference-best-theta` | UI 点「导出 Bundle」时勾选「优化 θ 寻参」(默认开),后端调用 `optimize_one_context` 360 次 |
...
POST /api/spawn-tuning-v2/policies/build-and-export
{
  "model_id": 40,
  "rollout_pct": 100,
  "optimize_theta": true,        # 主动启用 surrogate 寻参
...
- `d_step = state_d`(fillRate + actionFreedom + trend),θ 只影响"决策侧"(spawn 偏好 / 惊喜阈值),**不直接控制 fillRate 物理动力学**
...
5. **★ Bundle 必须写 best θ\*** — 否则模型学的 (ctx,θ)→curve 映射在部署阶段被丢弃,闭环断裂
6. **迭代精化** — 单轮 surrogate 极限有限,多轮"围绕 deployed θ\* 抖动采样"持续逼近
...
```python
# 纯真实棋盘状态, 不注入 target_S_curve (避免数据泄露)
...
与游戏页面 1:1 严格对齐 / 无 alias / 无历史枚举:

- DB generator 列仅 `CHECK IN ('rule', 'generative')`
- sampler 内部路径 `rule → 'baseline' (SPAWN_POLICY_RULES)`,调用与 game.js **同一** `generateDockShapes` 函数
...
**`📊 覆盖优先` Preset**:在 `🎯 高质量` 基础上进一步把采样**集中到够得着的高分段**——`lookahead-2` + 关 `random` bot + **仅采 `pb_bin=500/1500`**(实测 4000+ 时 `r_max≈0.57`,bot 完全够不着 PB)+ `12 θ × 5 seed × maxSteps 800`。
...
for (r, kind, target) in constraints:
    bin = int(r / bin_width + 1e-9)   # +1e-9 修浮点
...
**主导拉力**(权重 5.0)— 让 model 直接朝业务目标收敛,而非中间妥协态。
...
锚到 ideal `(head≈0.102, D_CAP=1.00)`,tol ±0.025(死锁)+ 权重 12.0(强力)→ **强制 model 端点贴 ideal**。低 r 长平台更轻,接近 PB 前后快速上冲,tail 仍保持 PB 后持续加压。
...
6. 验证指标      curve_mae, ideal_mae, curve_var, val_loss_breakdown × 12,
                 ...
                 ↓
7. EarlyStop     composite = ideal_mae + 0.3×endpoint + 0.2×anchor
...
**`DEPLOYABLE_*` 子集**等同于 GENERATOR_INDEX 全集:`rule / generative` + `random / clear-greedy / survival`,与游戏页面 1:1 对齐,360 ctx 即全量部署目标。`rl-bot` 仍是占位类别(UI 可勾选采样,不入 bundle)。
...
CREATE TABLE samples (
    ...
    generator       TEXT CHECK IN ('rule', 'generative'),
    ...
    algo_version    TEXT DEFAULT 'v2.10'
);
...
3. 检测 CHECK 约束是否严格 `('rule', 'generative')` → 不是则**重建表**:snapshot → DROP → schema.sql 重跑 → **只回填 generator IN ('rule', 'generative') 的样本**(老 enum 样本被 purge);UPDATE sample_sets.sample_count 重算
...
### 13.2 PB-aware d_step + Bayesian Prior

老公式 `d_step = state_d` 跟 r 无关,d_curve 平坦,model 无法学到 S 形
...
### 13.3 全链路对齐 ★ ideal target

- `target_fit / endpoint / anchor` **全部锚 ideal**(跨度 0.90)→ 三股力同向往 ideal 拉
- `L_shape` 权重 0.2,sample 仅作为防止 model 飞走的弱锚
- `val_ideal_mae` 是唯一 target MAE 指标
- 浮点 bug 修复:所有 `int(r / bin_width)` 改为 `int(r / bin_width + 1e-9)`(原本 `int(0.6/0.1)` 浮点误差得 5,正确应为 6)

### 13.4 Surrogate 寻参与迭代闭环

**根因**:`build-and-export` 如果直接写 `theta_norm=[0.5]*9` 进 bundle,模型学到的 `(ctx, θ) → d_curve` 映射**在部署阶段被丢弃**。结果即便 model 预测和 ideal 几乎重合(MAE 0.008),启发式实测 d_curve 仍是 default θ 跑出来的平坦曲线(MAE 0.27):
...
**部署侧**:`build-and-export` 加 `optimize_theta=true` 参数,对每 ctx 跑 `optimize_one_context`(8 starts × 300 Adam steps)在 model 上找 best θ*,写入 bundle(`build_mode=model-inference-best-theta`)。

**采集侧**:sampler 的 `thetas` 参数支持 `(ctx) => theta[]` 工厂函数。三种来源策略:
...
### 13.5 Loss 平衡 — 让 model 真学 ctx/θ 分布

朝 ideal 合力 vs 朝 sample 力如果过于悬殊(如 34:1),model 会收敛到"近似常数函数"(无视 ctx/θ,只输出 ideal_S)。后果:

- `predict-curve` 永远贴 ideal,跟实测均值 MAE 长期锁死
- 寻参时 model 对 θ 不敏感 → θ\* 信号弱
- 看似 model "完美",实际"什么都没学"
...
|------|----|------|
| `shape` | **5.0** | sample 主导 |
| `target_fit` | **0.5** | ideal 仅弱锚 |
| `endpoint` | **2.0** | 端点不再死锁,只是 hint |
| `anchor` | **3.0** | 仅保关键 r 点,中段放开 |
| `monotonic` | 2.5 | 不变 |
| `deploy` | 2.0 | 不变 |
...
### 13.6 物理侧 θ 接入 — 让 θ 真正撬动启发式

**问题**:`d_step = state_d` (仅 fillRate/freedom/trend),启发式物理上**跑不出 S 形**,实测 vs ideal MAE 长期锁死。模型/loss/寻参所有上层努力都被这个物理天花板锁住。

**解法**:让 θ 通过 PB-aware sigmoid 直接调制 d_step:
...
效果:
- state_d 恒定 0.5 时,d_curve 跨度可达 **~0.40**
- 启发式实测 d_curve 物理上具备 r 依赖的 S 形
- θ 寻参的物理空间被打开:不同 θ_center/width 让 d_curve 形状显著不同

### PB-aware 与 data leakage 的本质区别

| 项 | 旧 PB-aware (废弃) | 当前 PB-aware |
|--- |---|---|
...
| 1.0 | 100% PB-aware | ~70% | 完全等价 leakage,不推荐 |

#### 13.7 联合寻参 — 训练 + 寻参合并

**问题**:部署时跑 surrogate Adam (360 ctx × n_starts × steps),实测 ~90-180s。
...
**Fallback**:老 ckpt 无 `theta_optim_raw` 时,自动走 surrogate 路径(确保历史模型部署不破)。
...
### generator 与 game.js 严格 1:1(无 alias、无历史遗留)

清理了 legacy enum alias(`triplet-p1 / budget-p2 / heuristic-rule / generative`):

| 项 | 旧 (有 alias) | 新 (严格 1:1) |
|---|---|---|
...
修正了 `rule` 映射路径,改为 `'baseline'`(`SPAWN_POLICY_RULES`),simulator 内部直接调 `generateDockShapes(grid, layered, spawnContext)`,跟 game.js `_commitSpawn` 调的是**同一函数同一参数**。θ 通过 `modelConfig` 注入,等价于 game.js 通过 `resolveThetaV2` 拿 θ 后 `derivePbCurve`。
...
generator 与 game.js 严格 1:1 对齐(`rule / generative`),占位类别仅剩 `rl-bot`(训练可学,不入 bundle)
...
| 文件 | 内容 |
|---|---|
| `rl_pytorch/spawn_tuning_v2/target_curve.py` | ★ ideal S 曲线数学定义 |
...
### PB-aware d_step(纯真实状态, 不注入 target)
