...
1. [问题定义与设计哲学](#一问题定义与设计哲学)
2. [玩家画像数据模型](#二玩家画像数据模型)
3. [技能评估：从 raw 到 skillLevel](#三技能评估从-raw-到-skilllevel)
4. [历史融合：会话级与跨日记忆](#四历史融合会话级与跨日记忆)
5. [心流模型：F(t) 与三态判定](#五心流模型ft-与三态判定)
6. [挫败感与近失检测](#六挫败感与近失检测)
7. [认知负荷与动量](#七认知负荷与动量)
8. [会话阶段与节奏](#八会话阶段与节奏)
9. [玩法风格 (Playstyle) 检测](#九玩法风格-playstyle-检测)
10. [冷启动与置信度](#十冷启动与置信度)
11. [画像 → 出块的耦合](#十一画像出块的耦合)
12. [快照导出（用于 Bot/回放/商业化）](#十二快照导出用于-bot回放商业化)
13. [AbilityVector：模型化统一输出](#十三abilityvector模型化统一输出)
14. [完整公式速查](#十四完整公式速查)
15. [完整参数表](#十五完整参数表)
