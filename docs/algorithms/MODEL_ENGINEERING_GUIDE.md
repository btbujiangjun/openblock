...
> 若只需要四条核心玩法模型（启发式出块、生成式出块、PyTorch RL、浏览器 RL）的深度对照，先读 [`MODEL_SYSTEMS_FOUR_MODELS.md`](./MODEL_SYSTEMS_FOUR_MODELS.md)。
...
2. [共同工程假设](#二共同工程假设)
3. [端到端数据流](#三端到端数据流)
4. [RL 落子智能体](#四rl-落子智能体)
5. [Spawn 出块生成模型](#五spawn-出块生成模型)
6. [AdaptiveSpawn 自适应难度模型](#六adaptivespawn-自适应难度模型)
7. [PlayerProfile 与 AbilityVector](#七playerprofile-与-abilityvector)
8. [CommercialModelVector 与商业化决策](#八commercialmodelvector-与商业化决策)
9. [LTV 与运营策略模型](#九ltv-与运营策略模型)
10. [PCGRL 与候选生成](#十pcgrl-与候选生成)
11. [模型契约与特征字典](#十一模型契约与特征字典)
12. [训练、评估与上线检查](#十二训练评估与上线检查)
...
## 七、PlayerProfile 与 AbilityVector
...
## 八、CommercialModelVector 与商业化决策
