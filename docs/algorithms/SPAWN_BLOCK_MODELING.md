...
> 1. **逐 triplet 因果结果** `outcome`（**v1.69.1 `OUTCOME_DIM=15`**：旧 7 维不变 + 新增 `meanMoveRegret / meanMoveOptimality / forcedBadFlag / salvageFlag / roundAbsScore / 3×regrets`，由端侧 evaluation 写入 `ps.evalMetrics` 与 `ps.evalRound`，Python 离线直读不重算）。由 `_compute_spawn_outcome` 从 spawn→下一 spawn 间的 place 帧聚合；`weight` 叠加 `_outcome_weight_factor`（**v1.69.1 区间 [0.4, 2.0]**：salvage ×1.25 / forced_bad ×0.60 / 高 regret 线性扣 0.35）做因果微调。`outcome` 已进 `SpawnDataset.__getitem__`；`train_v3 --reward-blend ∈ [0,1]` 控制 reward 加权混入比例，默认 0 完全兼容旧训练。完整契约见 `SESSION_EVALUATION.md §9`。
