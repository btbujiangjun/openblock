...
| **SpawnTransformerV3.1** | 从对局日志学习 **条件分布** \(P(s_1,s_2,s_3 \mid \text{board}, \text{behaviorContext}, \text{history})\) | 显式消费玩家行为、能力向量、盘面拓扑与策略意图 | 需数据、需防「分数膨胀」等捷径；权重与旧 V3 不兼容 |
...
## 3. 学习路径：SpawnTransformerV3.1

实现见 `rl_pytorch/spawn_model/model_v3.py`、`dataset.py`、`train_v3.py`；前端上下文对齐见 `web/src/spawnModel.js`。V3.1 直接替换旧 V3 schema：`spawn_transformer_v3.pt` 需要按新结构重新训练，线上推理失败时仍回退规则轨。

### 3.1 建模思路

1. **行为克隆式的结构化输出**  
   训练标签来自真实对局：每一帧 `spawn` 事件记录 dock 三块形状 ID。模型学习在相同 **棋盘二值矩阵**、**56 维行为上下文**、**历史三连形状** 条件下，复现（并泛化）数据中的出块分布。
...
3. **多任务头：主预测 + 辅助约束**  
   ...
   - **意图头**：预测 `spawnIntent`（relief/engage/harvest/pressure/flow/maintain），让生成分布学习策略语义；
...
**（2）56 维行为上下文向量**（`dataset._parse_behavior_context`）  

V3.1 仍保留前 24 维基础画像，后 32 维显式纳入最新用户行为、能力向量、拓扑和策略意图：

| 索引 | 语义 | 说明 |
|------|------|------|
| 0–23 | 旧基础 context | 分数、填充率、技能、动量、情绪/认知、冷启动标志、窗口统计、长期能力、stress/flow/pacing/session |
| 24–31 | 数据可信度 + 拓扑 | `coldStart`、样本量、`boardDifficulty`、holes、nearFull/close/solutionCount |
| 32–37 | `AbilityVector` | `skillScore`、`controlScore`、`clearEfficiency`、`boardPlanning`、`riskTolerance`、`riskLevel` |
| 38–47 | `spawnTargets` + hints | 复杂度、解空间、清行机会、空间压力、payoff、新鲜度、保消、尺寸、多消、顺序刚性 |
| 48–53 | `spawnIntent` one-hot | relief / engage / harvest / pressure / flow / maintain |
| 54–55 | 额外策略上下文 | `multiLineTarget`、`sessionArc` |

枚举映射：`_FLOW_MAP`（bored/flow/anxious）、`_PACING_MAP`、`_SESSION_MAP`。`behaviorContext` 的前 24 维保留旧索引，是为了让反膨胀、playstyle 弱标签等训练 helper 可复用基础字段；V3.1 模型实际输入维度为 `BEHAVIOR_CONTEXT_DIM=56`。
...
**Token 构造**（序列长度 \(1+1+1+1+9=13\)）：

1. **CLS**：可学习向量 `cls_token`，聚合全局表示。  
2. **State token**：`[board_flat(64) ; behaviorContext(56)]` → `Linear(120→d_model)` + GELU + LayerNorm。  
3. **Difficulty token**：`target_difficulty (1)` → `Linear(1→d_model)` + GELU + LayerNorm。  
4. **Style token**：`playstyle_id` → `Embedding(NUM_PLAYSTYLES, d_model)`，控制个性化倾向。  
5. **History tokens（9 个）**：每个 cell为形状 ID → `Embedding(NUM_SHAPES+1, d_model)`，加 `history_pos`，`padding_idx=NUM_SHAPES`（与实现一致）。
...
| 维度 | 规则 | SpawnTransformerV3.1 |
|------|------|---------------------|
| 可解性保证 | 强（高填充 DFS） | 弱（数据+损失间接） |
| 可解释性 | 强（`_spawnDiagnostics`） | 弱（需归因工具） |
| 个性化 | 依赖画像→stress→权重 | 直接吃 56 维行为上下文 + 历史 + playstyle/LoRA |
