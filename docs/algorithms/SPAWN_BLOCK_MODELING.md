...
5. **多任务辅助头：抑制捷径学习**  
   ...
   - **意图头（自监督）**：预测 `spawnIntent`（`relief/engage/harvest/pressure/flow/maintain/sprint`，v1.57.1 新增 sprint 共 7 类），让生成分布学习策略语义；
...
>
> **v1.61.0 治理**：除了上述「θ→spawnTargets」隐式路径，现已把 4 个 PB θ 作为 `behaviorContext[57–60]` 的**显式条件输入**（见 §3.3），使模型对 θ 泛化；并以 `drift.py` 的 PSI 漂移监控做兜底。两者合力把「换 θ 不重训」的分布漂移退化风险降到最低。
...
**（2）61 维行为上下文向量**（`dataset._parse_behavior_context`，v1.57.1：56→57；v1.61.0：57→61）  
...
| 48–54 | `spawnIntent` one-hot（**7 维**） | relief / engage / harvest / pressure / flow / maintain / **sprint**（v1.57.1 新增） |
| 55–56 | 额外策略上下文 | `multiLineTarget`、`sessionArc` |
| 57–60 | **PB 曲线 θ 显式条件**（v1.61.0） | `pbTensionCenter / pbTensionWidth / pbBrakeCenter / pbBrakeWidth`，按 `_PB_THETA_RANGES` 归一化；来源 `ps.adaptive.stressBreakdown.pbCurveParams`，缺省 → 默认 θ 域 |
...
> 🧭 **v1.61.0 显式 θ 条件（把 L2→L1-Net 隐式耦合转为显式输入）**：把本帧实际生效的 4 个 PB 曲线 θ 直接喂进网络（[57–60]），让模型对 θ **泛化**而非把 θ 当作不可见的分布漂移源——这是消除「换 θ 不重训 → 行为克隆假设失效」退化的治本手段。配套样本元数据 `theta_regime`（int，不进网络）供**分层重训 / 漂移分组**；运行期分布漂移由 `rl_pytorch/spawn_model/drift.py`（`spawn_targets_drift / pb_theta_drift / assert_spawn_targets_drift`，PSI）做部署门禁兜底。
...
| 头 | 输入 | 维度 | 作用 |
|----|------|------|------|
| `head_0` | `cls_out` | `NUM_SHAPES`（**40**，v1.60.0：28→40，含 12 个 2-3 格新形状） | 第 0 槽 logits |
...
- **产物**：`models/spawn_transformer_v3.pt`（含 `model_state_dict`、`config` 超参、`model_version='v3.1-behavior'`、`context_dim=24`、`behavior_context_dim=61`、`num_shapes/num_categories/num_playstyles/num_spawn_intents`、`epoch/val_loss/val_acc`、v1.61.0 起还含 **`drift_reference`**（训练集 spawnTargets/PB θ 画像）与 **`theta_regimes`**（θ-regime 分布））；进度 JSON `models/spawn_train_status.json` 供训练面板展示。

> 🧩 **v1.63 数据集补全（从「模仿」到「可优化」）**：配合 `PLAYER_STATE_SNAPSHOT_VERSION=3`，把"出块算法优化"真正需要的标签补齐——
...
- **分布漂移监控**（`drift.py`，v1.61.0）：对 behaviorContext 的 `spawnTargets` 段（[38:44]）与 PB θ 段（[57:61]）计算 PSI（`population_stability_index`），`assert_spawn_targets_drift` 可作为重训/部署门禁（经验阈值 0.10 关注 / 0.25 重训）；配合显式 θ 条件防止 L2 换 θ 后 Net 的 OOD 退化。自检见 `test_drift.py`。
- **部署门禁 CLI**（`drift_check.py`，v1.61.0）：`python -m rl_pytorch.spawn_model.drift_check --db <db> --ckpt <pt>` 用 checkpoint 内 baked-in 的 `drift_reference` 对照线上 behaviorContext，漂移超阈值退出码 1。这是**数据漂移门禁**，需线上 DB + ckpt，属**部署期/发布前**检查（不进 CI，因 CI 无生产数据）；npm 别名 `npm run spawn:drift-check -- --db <db> --ckpt <pt>`。训练侧 `train_v3.py` 会打印 θ-regime 分布并与上一版 ckpt 的参考画像做非致命漂移对照。
- **CI 回归门禁**（`.github/workflows/ci.yml` → job `spawn-model-gate`，v1.61.0）：每次 push / PR 安装 CPU 版 torch + numpy 后跑 `test_drift`（PSI / θ 归一化 / 61 维契约 / 漂移参考构建）与 `test_v3`（61 维前向、`board_proj=125`、AR 头、LoRA 往返）。守护的是**逻辑与张量契约不回退**；web↔python 维度 parity 由 vitest 覆盖。本地等价命令：`npm run spawn:gate`。
- **显式 θ 日志就绪度门禁**（`theta_readiness.py`，v1.61.0）：`python -m rl_pytorch.spawn_model.theta_readiness --db <db>`（npm 别名 `spawn:theta-readiness`）。重训**前置**检查 `ps.adaptive.stressBreakdown.pbCurveParams` 的三项指标：覆盖率、θ-regime 数、归一化后跨度。任一不达标（默认 80% / ≥2 / ≥0.05）→ 退出码 3，说明显式 θ 那 4 列接近常数、重训学不到 θ→出块映射（此时显式 θ 仅为安全网，需先让 L2 tuner 产出有差异的 PB 曲线 θ 并累计新日志）。属数据相关的部署/重训前门禁，不进 CI。
...
| 2026-05-29 | v1.63 接入 B.2 样本集库：行为样本集（主库 `spawn_dataset_samples`）与 v2 寻参样本（独立 `spawn-tuning-v2.sqlite` 的 `sample_sets/samples`，d_curve 标签）异构、分库，故不入 `samples` 表，而由 v2 后端以哨兵 `set_id=900000001` 合成"用户行为样本集"行注入 `/sample-sets` 列表（自动同步名称/样本数/状态/时间）；预览/下载走主库专属视图（每行一局含 frames），d_curve 专用的质量/分析/训练对行为集禁用（前端隐藏 + 后端 sentinel 守卫 403）。回填存量经 `POST /api/spawn-dataset/sync` |
| 2026-05-29 | v1.63 改进：①采样时 PB 围绕"指定数值"波动（`PB_JITTER_DEFAULT=0.15`，避免常量），reward 口径按采样 PB（`_pb_reward`），样本加 `pb_sampled/pb_ratio_sampled/reward`；②行为序列打包成 append-only 样本集 `spawn_dataset_samples`（自动同步 UPSERT、`BEFORE DELETE` WORM 触发器禁删、payload 存 frames 副本去耦），`load_packed_dataset()` + `load_training_data(prefer_packed=True)`，端点 `/api/spawn-dataset/sync|stats`（详见 §3.5 改进①②） |
| 2026-05-29 | v1.63：出块数据集补全（模仿→可优化）。snapshot pv 2→3：新增 `bestScore/pbRatio`、`spawnGeo.nearFullLines/close1/close2/maxColHeight`、逐 spawn `provenance`、dock 逐块 `feat` + 帧级 `spawnMeta`；`dataset.py` 新增逐 triplet `outcome`（7 维因果结果）+ `_outcome_weight_factor` 加权、局级 `game_over_reason/died` 标签、`load_session_retention()` 留存 join；server `sessions.game_over_reason` 列 + `endGame` 死亡盘面落库（详见 §3.5「v1.63 数据集补全」） |
| 2026-05-29 | v1.61.0：behaviorContext 57→61，尾部加 **4 维 PB θ 显式条件**（把 L2→L1-Net 隐式耦合转显式，治本防分布漂移）+ 样本 `theta_regime` 元数据；新增 `drift.py` + `drift_check.py`（spawnTargets/PB θ PSI 漂移监控 + 部署门禁），ckpt baked-in `drift_reference`；`train_v3` 打印 θ-regime 并与旧 ckpt 漂移对照；board_proj 121→125。已用 8473 样本重训 `spawn_transformer_v3.pt`（单一 θ-regime，门禁通过）。前后端契约（含 `SPAWN_PB_THETA_RANGES`）由 parity 测试钉死；新增 CI job `spawn-model-gate`（CPU torch 跑 `test_drift`+`test_v3` 回归门禁）与 npm 别名 `spawn:gate`/`spawn:drift-check` |
