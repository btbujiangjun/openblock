...
| 出块寻参离线包 (v0.3.7) | `web/public/spawn-tuning/policies.json`、`miniprogram/core/tuning/spawnPolicies.js` | 由 `POST /api/spawn-tuning/v2/policies/bundle/export` 同时写两端;Web/Android/iOS 在 `clientPolicy.loadPoliciesFromBundle()` 启动时优先读,小程序通过 sync-core.sh 转 CJS 数据模块;`policies.meta.json` 含 SHA-256 / 生成时间 / run_id,可在 CI 校验 |
