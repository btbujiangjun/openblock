...
| 出块寻参离线包 (v2) | `web/public/spawn-tuning-v2/policies.json`、`miniprogram/core/tuning/v2/spawnPoliciesV2.js` | 由 `POST /api/spawn-tuning-v2/policies/bundle/export` 同时写两端;Web/Android/iOS 在 `clientPolicyV2.loadPoliciesFromBundle()` 启动时优先读,小程序通过 sync-core.sh 转 CJS 数据模块;`policies.meta.json` 含 SHA-256 / model_id / 生成时间, 可在 CI 校验 |
