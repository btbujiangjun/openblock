...
| 维度 | 数值 | 影响 |
|------|------|------|
| 状态空间大小 | $\sim 2^{64}$（每格 0/1）× 28³ dock 组合 | 巨大但稀疏 |
...
搜索增强：     可选 1-step / 2-ply beam / MCTS（用于 Q 蒸馏）
...
路径 A: Python 自博弈（高吞吐）
   ...
   │     ↓                                                        │
   │  攒 batch_episodes=8 局 → _reevaluate_and_update              │
   │     ↓                                                        │
   │  PPO 更新（多 worker 可选）                                   │
   │     ↓                                                        │
...
路径 B: 浏览器自博弈（在线）
   ...
   │     ↓                                                        │
   │  每步 POST /api/rl/select_action (推理)                       │
   │     ↓                                                        │
...
`s ∈ ℝ^162`，由 `extract_state_features(grid, dock)` 产出，**双端一致**（`features.py` / `features.js`）。

### 3.1 拆分

```
s = [scalars(23) ; grid_flat(64) ; dock_flat(75)]
            ↑              ↑              ↑
...
```
shared/game_rules.json featureEncoding.stateDim = 162
                       ↓
...
```python
outcome = clip(final_score / win_threshold, 0, 2)
return_target = (1 - mix) · GAE_return + mix · outcome
```

`mix = 0.5` 默认（`outcomeValueMix.mix`）。所有时刻共享同一 outcome 终局信号。

### 10.3 直观

```
GAE：     "根据 V 估的，可能不准但有时序信息"
Outcome： "终局这一局打了多少分（除以胜利门槛）"
...
| `feasibility` | 是否仍可放任何块 | trunk 学会"是否还能继续" |
...
```python
{
    ...
    'feasibility': 1 if has_legal_actions else 0,
