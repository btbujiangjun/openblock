...
> 的四、改进项编号，再提交代码 PR。
>
> **诚实标注前置**：本文档严格区分"已实现"与"仅文档 / 未落地"两类——商业化触发护栏、外围系统中的剧情 / 世界观模块、北极星 6 项细指标的 SQL 落地等均存在不同程度的"文档先行 / 实现待补"情况，详见 §十二 与 §十三。

## 阅读顺序

| 节 | 回答的问题 | 适合角色 |
|---|---|---|---|
| [一、核心主线](#一核心主线挑战个人最佳) | OpenBlock 的玩法目标是什么？为什么"PB 追逐"可以贯穿 5 层结构？ | 主策划、产品总监、新人破冰 |
| [二、体验五轴](#二体验五轴挑战能力--节奏报偿--掌控公平--情感记忆--成长身份) | 产品如何把"体验"分解为可观察的 5 条通路？ | 产品策划、体验设计 |
| [三、生命周期 × 成熟度 25 格](#三生命周期--成熟度-25-格压力上限映射) | 同一棋盘上，不同玩家凭什么得到不同的难度曲线？ | 主策划、运营、算法 |
| [四、核心玩法层（短循环 6 步）](#四核心玩法层短循环-6-步) | 玩家每一回合的 6 步操作分别落在哪个文件？ | 产品、QA、新工程师 |
| [五、体验调控层](#五体验调控层实时调制) | 压力 / 出块意图 / 出块提示 / 节奏 4 条调制通路在哪？ | 产品、算法 |
| [六、策略辅助层](#六策略辅助层认知支持) | 策略卡 / 拓扑洞察 / 瓶颈识别 / 多消机会如何被生成？ | 产品、算法 |
| [七、目标成长层](#七目标成长层长期动机) | PB 追赶 / 里程碑 / 段位 / 经验 / 关卡 5 条长期动机如何叠加？ | 主策划、运营 |
| [八、情感反馈层](#八情感反馈层情绪闭环) | 多消 / 完美清台 / 近失 / 新纪录 / 震动 / 特效 / 叙事如何被触发？ | 产品、体验、QA |
| [九、外围系统（可关闭）](#九外围系统可关闭不抢戏) | 任务 / 签到 / 赛季 / 通行证 / 社交 / 异步 PK 入口在哪？ | 产品、运营 |
| [十、商业化边界（可关闭）](#十商业化边界可关闭体验优先) | 广告 / 内购 / 礼包如何在不损害体验前提下接入？ | 商业化、运营 |
| [十一、数据与迭代闭环](#十一数据与迭代闭环) | 玩家行为 → 行为数据 → 决策调整 → 行为结果如何形成正反馈？ | 产品、数据、算法 |
| [十二、核心评估指标（北极星）](#十二核心评估指标北极星) | "调到追逐 PB 但不轻易超越 PB"的甜区如何被量化？ | 产品总监、运营、数据 |
| [十三、设计哲学](#十三设计哲学与行业默认做法不同的具体选择) | 产品层面与行业默认做法不同的 4 处具体选择是什么？ | 全角色 |
...
>
> 本图是**最高层的产品视角**：不出现具体代码模块，把整个产品浓缩为一根核心主线 + 5 层游戏化结构 + 25 格压力调制矩阵 + 一组北极星指标。下方 §一–§十二 把图中每一块映射到具体的代码入口与文档锚点。
...
| PB 与压力的耦合（B 类挑战加压） | `web/src/adaptiveSpawn.js` `isBClassChallenge` 在 `currentScore / bestScore ≥ 0.8` 时触发 `challengeBoost`；8 段 bypass 见 §五 |
...
## 二、体验五轴：挑战能力 / 节奏报偿 / 掌控公平 / 情感记忆 / 成长身份
...
短循环不进行 ML 推理，所有 ML / 自适应决策发生在 ② 选块前与 ⑥ 下一组前，由下方 §五–§六 调制。
...
| 动机 | 代码入口 |
|---|---|
| **最佳分追赶** | §一 已展开（`bestScoreBuckets.js` + `_maybeCelebrateNewBest`） |
...
- **压力抑制期降变现强度**：与 §五 `postPbReleaseStressAdjust` 协同——破 PB 后 3 个 spawn 内不应插播打断；
- **PB 异常分守卫**：与 §一 `endGame` sanity guard 协同——疑似异常分不计入 LTV 计算口径。
