...
| 你的角色 | 必读 | 选读 | 跳过 |
|---------|-----|------|------|
| **算法工程师 · 新人** | 本手册 § 2~4 → [`ALGORITHMS_RL.md`](./ALGORITHMS_RL.md) → [`ALGORITHMS_PLAYER_MODEL.md`](./ALGORITHMS_PLAYER_MODEL.md) | [`SPAWN_BLOCK_MODELING.md`](./SPAWN_BLOCK_MODELING.md) | 商业化（除非负责） |
...
| **算法工程师 · 商业化/CRM** | [`ALGORITHMS_MONETIZATION.md`](./ALGORITHMS_MONETIZATION.md) + [`MONETIZATION_TRAINING_PANEL.md`](./MONETIZATION_TRAINING_PANEL.md) | [`MONETIZATION_CUSTOMIZATION.md`](./MONETIZATION_CUSTOMIZATION.md) | RL |
| **数据/分析** | [`ALGORITHMS_PLAYER_MODEL.md`](./ALGORITHMS_PLAYER_MODEL.md) + [`ALGORITHMS_MONETIZATION.md`](./ALGORITHMS_MONETIZATION.md) | KPI 章节于 [`MONETIZATION_TRAINING_PANEL.md`](./MONETIZATION_TRAINING_PANEL.md) | RL 训练细节 |
| **游戏/玩法工程师** | [`ALGORITHMS_SPAWN.md`](./ALGORITHMS_SPAWN.md) + [`ADAPTIVE_SPAWN.md`](./ADAPTIVE_SPAWN.md) + [`CLEAR_SCORING.md`](./CLEAR_SCORING.md) | [`ALGORITHMS_PLAYER_MODEL.md`](./ALGORITHMS_PLAYER_MODEL.md) | RL/商业化 |
| **运营** | [`MONETIZATION_TRAINING_PANEL.md`](./MONETIZATION_TRAINING_PANEL.md) | [`STRATEGY_GUIDE.md`](./STRATEGY_GUIDE.md) | 公式细节 |
...
| **B. 计分** | 消行得分公式 | 二次方 + Bonus 线 | [`CLEAR_SCORING.md`](./CLEAR_SCORING.md) |
...
| 状态特征维度 (162) / 动作维度 (12) / φ 维度 (174) | `shared/game_rules.json` `featureEncoding` | `features.js` + `features.py` 同步；checkpoint **失效**需重训 |
...
| 符号 | 含义 | 取值 | 来源 |
|------|------|------|------|
| $s$ | 棋盘状态（特征向量） | $\mathbb{R}^{162}$ | `extract_state_features` |
| $\psi(a)$ | 动作特征 | $\mathbb{R}^{12}$ | `extract_action_features` |
| $\phi(s,a)$ | 状态-动作联合特征 | $\mathbb{R}^{174}$ = $[s; \psi(a)]$ | `build_phi_batch` |
...
网络：    ConvSharedPolicyValueNet (~182K 参数, width=128)
          - CNN(32) + 2×ResConv 编码 8×8 棋盘
          - DockBoardAttention：dock(3 槽) 对 grid 特征做交叉注意力
          - 三段拼合 → trunk(128) → policy / value / 三辅助监督头
状态：    s ∈ ℝ¹⁶² (23 标量 + 64 grid + 75 dock)
...
推理：    POST /api/rl/select_action → 单次 ms 级；可选 1-step lookahead
训练：    python -m rl_pytorch.train（Python 自博弈）
          或浏览器 → /api/rl/train_episode（在线 PPO）
```
...
### 5.5 计分（[`CLEAR_SCORING.md`](./CLEAR_SCORING.md)）
...
| v6/v7 (实验) | 路线图 | + Q 蒸馏 / 2-ply Beam / 评估门控 | — | — |
...
| 算法分册 | 关联实现文档 | 关联运营文档 |
|---------|------------|------------|
| [`ALGORITHMS_RL.md`](./ALGORITHMS_RL.md) | `RL_AND_GAMEPLAY.md` / `RL_ANALYSIS.md` / `RL_TRAINING_NUMERICAL_STABILITY.md` / `RL_TRAINING_OPTIMIZATION.md` / `RL_ALPHAZERO_OPTIMIZATION.md` / `RL_BROWSER_OPTIMIZATION.md` / `RL_TRAINING_DASHBOARD_FLOW.md` / `RL_TRAINING_DASHBOARD_TRENDS.md` | — |
| [`ALGORITHMS_PLAYER_MODEL.md`](./ALGORITHMS_PLAYER_MODEL.md) | `PLAYER_ABILITY_EVALUATION.md` / `PANEL_PARAMETERS.md` / `REALTIME_STRATEGY.md` / `PLAYSTYLE_DETECTION.md` | — |
| [`ALGORITHMS_MONETIZATION.md`](./ALGORITHMS_MONETIZATION.md) | `MONETIZATION.md` / `MONETIZATION_CUSTOMIZATION.md` | `MONETIZATION_TRAINING_PANEL.md` / `COMMERCIAL_OPERATIONS.md` |
| [`ALGORITHMS_SPAWN.md`](./ALGORITHMS_SPAWN.md) | `SPAWN_ALGORITHM.md` / `ADAPTIVE_SPAWN.md` / `SPAWN_BLOCK_MODELING.md` | `DIFFICULTY_MODES.md` |
| `CLEAR_SCORING.md`（已存在） | — | — |
...
> 最后更新：2026-04-27 · v1 · 与 RL v5 / 商业化 v3 同步  
> 维护：算法工程团队
