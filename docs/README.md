...
- [性能基线与回归检测](./engineering/PERFORMANCE.md#性能基线与回归检测) —— CPU自动/GPU手动基线，`npm run perf:check`，敏感模块列表
...
- [微信小程序发布流程](./platform/WECHAT_MINIPROGRAM.md#微信小程序发布流程) —— 账号注册→代码上传→审核，完整Runbook
...
- [商业化定制](./operations/MONETIZATION.md#商业化策略定制指南) —— 三级定制粒度：面板调参→规则修改→新增动作类型，热重载机制
