...
- [🎭 全链路实践演示文稿](./PRESENTATION.html) —— **交互式 HTML 演示文稿**：策划与算法工程师对话体，图标化高密度信息呈现，覆盖品类分析、体验设计方法论、出块引擎9层演化、玩家画像、强化学习、商业化与架构全景
...
- [难度模式](./product/DIFFICULTY_MODES.md) —— 三档难度（简单/普通/困难）的参数差异；v1.68 起含局间难度（RoR）小节
- [局间难度（RoR）](./algorithms/ALGORITHMS_SPAWN.md#十六局间难度ror) —— v1.68 新增"今日第几局/距离上次休息多久"维度：5 档 arc × humped 连战曲线 × arc-aware D 曲线形变，含 5×5×5 立方矩阵
- [消行计分规则](./product/CLEAR_SCORING.md) —— Clear / Combo / Streak 公式、同花色/同icon奖励、全消倍率、web与小程序对齐口径
- [实时策略系统](./algorithms/REALTIME_STRATEGY.md) —— L1指标字典（20+滑动窗口）、stress管线公式、6档压力表、策略卡生成、合理性评估清单（v1.68 起归类至算法工程师目录，供产品/策划只读引用）
- [玩法风格检测](./algorithms/REALTIME_STRATEGY.md#玩法偏好识别与出块联动) —— 多消率/清屏率/平均消除条数推算，`playstyle`枚举与出块对齐
...
- [出块算法手册](./algorithms/ALGORITHMS_SPAWN.md) —— **出块统一手册**：双轨架构 + `SpawnTransformer` 网络/训练/推理（§1–§11），整合架构分层（§12）、出块建模与候选块概率图鉴（§13）、难度与评估（§14）、参数寻优 SpawnParamTuner（§15）、局间难度 RoR（§十六）、温暖局 Warm Run（§十七）
- [自适应出块](./algorithms/ADAPTIVE_SPAWN.md) —— 运行时深潜：§10.8完整流水线（9层v1.60.35代码基准）+ S×M×D×P 25格调制矩阵
- [**局间难度（RoR）**](./algorithms/ALGORITHMS_SPAWN.md#十六局间难度ror) —— 5 档 arc 派生流程图、humped 连战曲线、5×5×5 立方调制矩阵、arc-aware D 曲线形变、跨语言契约
- [**温暖局（Warm Run）**](./algorithms/ALGORITHMS_SPAWN.md#十七温暖局warm-run) —— v1.70 新手/回流/连挫人群保护：7 触发器 × 3 强度档 × 爽感编排 × 预算渐退
- [**实时策略系统：信号流与出块链路**](./algorithms/REALTIME_STRATEGY.md) —— L1 指标字典（thinkMs/pickToPlaceMs/clearRate 等 20+ 滑动窗口）→ L2 stress 管线公式 → L3 策略解析 → L4 压力表与策略卡决策树；含玩法风格识别（`perfect_hunter`/`multi_clear`/`combo`/`survival`/`balanced`）与出块联动机制、合理性评估清单
- [**单步与单轮放块质量评估**](./algorithms/PLACEMENT_QUALITY.md) —— v1.69 evaluatePlacement / evaluateRound：5 维分量、regret/badnessTag、order/path/payoff 三类 regret 拆解、forced_bad/salvage 公平性门控
- [**单局与局间评估**](./algorithms/SESSION_EVALUATION.md) —— v1.69 sessionEvalRecord schema、`evaluation_session` 表、`/api/evaluation/{session,sessions,ror_audit}` API、与 v1.68 RoR 系统的双向耦合验证矩阵、A/B 自动回滚门
- [出块算法信号透视仪](./algorithms/spawn-signal-explorer.html) —— 交互式 HTML：L1/L2/L3 信号浏览器 + v1.66 压力阶段 + v1.67 构造式出块（代码口径 v1.67）
