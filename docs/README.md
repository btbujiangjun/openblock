...
OpenBlock 把上述场景所需的支柱——自适应出块、双层玩家画像（生命周期 + 成熟度）、PyTorch + MLX 双训练栈、5×5（25 格）双轴运营策略矩阵、四端同步底座——一次性放在同一个仓库里，并用契约文档锁住模块间的依赖方向。下面四个架构视角分别从**业务、产品、系统、算法**展开：业务视角看四支柱如何串起完整玩家旅程；产品视角看核心循环、个人最佳分挑战、体验五轴与生命周期分层如何构成可运营的游戏产品；系统视角看跨端、可观测、合规、安全如何被前置为架构默认值；算法视角看从信号采集到 RL 自博弈的六层工序如何共享同一组特征定义。
...
## 二、产品视角：从核心循环到个人最佳分挑战的体验架构

> **写给主策划人、产品负责人、体验设计师与策略设计师**——
> 帮助你判断：这款游戏为什么能反复玩？玩家每局追求什么？系统如何让“挑战自己的最佳分”既有可见进展、又不被轻易突破？不同生命周期和成熟度玩家为什么需要不同的产品策略？

OpenBlock 的产品架构不是“玩法 + 若干外围功能”的堆叠，而是围绕 **拖拽放置 → 消行反馈 → 压力变化 → 个人最佳分追赶 → 结算复盘 → 再来一局** 的短循环，把体验目标、策略算法、元进度和运营触达接在同一条玩家旅程上。

| 产品层 | 玩家问题 | 系统能力 | 主要入口 |
|---|---|---|---|
| **核心玩法层** | “我一眼能懂、马上能玩、每一步有反馈吗？” | 8×8 棋盘、3 块 dock、拖拽落子、消行计分、失败判定、即时特效 | `web/src/game.js`、`grid.js`、`renderer.js`、[消行计分](./product/CLEAR_SCORING.md) |
| **体验调控层** | “这局为什么变紧、为什么又给我喘息？” | stress、flowState、frustration、pacing、near-miss、spawnIntent、策略卡 | [实时策略系统](./player/REALTIME_STRATEGY.md)、[策略体验栈](./player/STRATEGY_EXPERIENCE_MODEL.md) |
| **目标与成长层** | “我是在进步，还是只是随机多玩一局？” | 个人最佳分、相对里程碑、bestScoreChase、XP/等级、成熟度晋升 | [挑战个人最佳分策略设计](./player/BEST_SCORE_CHALLENGE_STRATEGY.md)、`progression.js` |
| **情感与记忆层** | “这局有没有爽点？失败后我还想回来吗？” | 多消、Perfect Clear、同 icon bonus、皇冠新纪录、近失 toast、结算复盘 | [彩蛋与惊喜](./product/EASTER_EGGS_AND_DELIGHT.md)、`nearMissPlaceFeedback.js` |
| **生命周期运营层** | “我是新手、成长玩家、核心玩家还是回流玩家？” | S0–S4 × M0–M4 分层、stress cap、回流保护、任务/活动/触达 | [生命周期与成熟度蓝图](./operations/PLAYER_LIFECYCLE_MATURITY_BLUEPRINT.md) |
| **商业化边界层** | “变现是否服务体验，而不是打断体验？” | IAA/IAP feature flag、事件总线、offer、签到、宝箱、钱包、频控 | [商业化系统全景](./operations/MONETIZATION.md)、[宝箱与钱包](./product/CHEST_AND_WALLET.md) |

**产品北极星**：让玩家在多数有效会话中看到“接近自己最佳分”的希望，在少数高质量会话中完成真实突破，并在失败后知道下一局为什么可能更好。这个北极星落到三条产品约束：

- **短循环必须成立**：单局不依赖外网，不依赖元系统也可完整玩完；元进度只是下一局动机，不替代核心消除爽点。
- **挑战不能廉价**：个人最佳分进入 `scoreStress`、`challengeBoost` 和 `bestScoreChase`，但受 `boardRisk`、`bottleneckRelief`、`frustrationRelief`、S/M 生命周期调制保护。
- **复盘必须可解释**：玩家临近新高却失败时，系统应能从 holes、firstMoveFreedom、clearRate、pickToPlaceMs、nearFullLines 等事实解释“差在哪里”，而不是只弹“再来一局”。

**产品架构的长线回报**：

- **主策划能按体验目标调系统**：想增加“冲刺新高”的紧张感，不是拍脑袋提高难度，而是调整 best-zone、orderRigor、solutionSpacePressure、UI 叙事和破纪录频率护栏。
- **策略设计能按玩家阶段差异化**：S0/M0 是建立信心，S2/M2 是技能成长，S3/M4 是稀缺荣誉，S4 是恢复旧水平；同一个 90% bestRatio 不再使用同一套话术和加压。
- **产品、算法、运营共用同一语言**：体验五轴、bestScoreChase、S/M 标签、spawnIntent、stressBreakdown、replayAnalysis 都能互相映射，避免产品说“爽一点”、算法只看到“stress +0.1”。

后续若要生成产品架构图，可直接使用 [产品架构图生成 Prompt](./product/PRODUCT_ARCHITECTURE_DIAGRAM_PROMPT.md)；它给出产品北极星、玩家旅程、功能分层、体验五轴、best-score chase 与 S/M 生命周期分群的完整事实包。
...
> 上节四个架构视角说明"它由什么构成"。本节列出三处**与行业默认做法不同的具体选择**，并标注随之而来的代价与回报，便于读者判断这些取舍是否适合自己的项目。
...
| [产品架构图生成 Prompt](./product/PRODUCT_ARCHITECTURE_DIAGRAM_PROMPT.md) | 重生成产品架构图的可复用 prompt 模板与事实包 | 产品、主策划、策略设计、AI 协作 |
...
| [挑战个人最佳分策略设计](./player/BEST_SCORE_CHALLENGE_STRATEGY.md) | 围绕“挑战自己但不轻易破纪录”的体验结构、当前策略审计、生命周期/成熟度分层与优化路线 | 主策划、策略设计、算法 |
| [产品架构图生成 Prompt](./product/PRODUCT_ARCHITECTURE_DIAGRAM_PROMPT.md) | 围绕玩家旅程、核心循环、体验五轴、最佳分挑战和生命周期分层生成产品架构图 | 主策划、产品、策略设计、AI 协作 |
...
- **策略与叙事一致性**：先读 [体验设计基石](./player/EXPERIENCE_DESIGN_FOUNDATIONS.md)
  → [挑战个人最佳分策略设计](./player/BEST_SCORE_CHALLENGE_STRATEGY.md) →
