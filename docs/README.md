...
| **算法 / ML 工程师** | 启发式、生成式、强化学习三栈共存且共享特征定义的工程模板，把"算法漂移"与"训-推不一致"作为可被预防的工程问题处理 | [算法与模型手册](./algorithms/ALGORITHMS_HANDBOOK.md) → [出块评估与可视化工具](./algorithms/SPAWN_EVALUATION.md) → [RL 算法手册](./algorithms/ALGORITHMS_RL.md) |
...
| 算法工程师 | [算法架构图](./algorithms/ALGORITHM_ARCHITECTURE_DIAGRAMS.md) → [算法与模型手册](./algorithms/ALGORITHMS_HANDBOOK.md) → [四模型系统设计](./algorithms/MODEL_SYSTEMS_FOUR_MODELS.md) | [出块算法手册](./algorithms/ALGORITHMS_SPAWN.md) → [出块评估与可视化工具](./algorithms/SPAWN_EVALUATION.md)（P1/P2、个性化、Worker、SQLite/本地降级、DFV 口径） → [玩家画像算法](./algorithms/ALGORITHMS_PLAYER_MODEL.md) → [RL 手册](./algorithms/ALGORITHMS_RL.md) → [商业化模型架构设计](./algorithms/COMMERCIAL_MODEL_DESIGN_REVIEW.md) |
...
| 测试 / QA | [测试指南](./engineering/TESTING.md) → [性能基线与回归检测](./engineering/PERFORMANCE_BASELINE.md) | [RL 数值稳定](./algorithms/RL_TRAINING_NUMERICAL_STABILITY.md) → [训练看板趋势](./algorithms/RL_TRAINING_DASHBOARD_TRENDS.md) |
...
| [性能基线与回归检测](./engineering/PERFORMANCE_BASELINE.md) | CPU 基线 (`npm run perf:check`) + GPU 基线 (`window.__perfOverlay.startProfile`)，防止性能退化 |
