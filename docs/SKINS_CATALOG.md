# OpenBlock 皮肤目录（35 款全集）


> 版本: v10.33 | 更新: 2026-04-30
...
OpenBlock 提供 **35 款** 主题皮肤，覆盖 4 大维度：

- **明暗底**：31 款深色 + 4 款浅色（`dawn` / `pets` / `farm` / `desert`）
- **是否携带 emoji icon**：27 款带 icon（每款 8 枚）+ **8 款纯配色**
...
合计 **35** 款。默认皮肤为 `titanium`（钛晶矩阵），由 `DEFAULT_SKIN_ID` 常量控制；玩家选择持久化至 `localStorage.openblock_skin`。

**已下线与迁移**（`REMOVED_SKIN_ALIASES`，读档时自动改写）：`cyber`→`neonCity`，`macaroon`→`dawn`，**`neural`→`neonCity`（v10.32）**。

**v10.31**：剔除无 icon 重复赛道 `cyber`、`macaroon`。**v10.32**：剔除带 icon 皮肤 `neural`（人工智能），暗色科技仅剩 `aurora` / `neonCity`，算力叙事并入霓虹都市款。
...
`classic`（中性灰盘） · `titanium`（钛冷蓝） · `aurora`（北极冰蓝） · `neonCity`（都市夜近黑） · `ocean`（深海蓝） · `lava`（焦炭黑） · `koi`（深水蓝） · `bubbly`（深紫海） · `toon`（漫画聚光紫） · `pixel8`（街机厅黑） · `dawn`（晨光奶油） · `food`（餐厅暖棕） · `music`（演出暗紫） · `pets`（家居米黄） · `universe`（深空近黑） · `beast`（荒野焦土） · `industrial`（焦煤铸铁） · `forbidden`（玄朱朱红） · `sports`（草绿球场） · `outdoor`（黎明山谷蓝，v10.17.4 新增即合规） · `vehicles`（机库灰混凝土） · `forest`（苔藓深绿） · `pirate`（深海舷板） · `farm`（浅春绿牧草） · `desert`（浅沙金） · `mahjong`（茶馆绿呢+实木暖灯，v10.17.3 重制） · `boardgame`（赌场红丝绒+绿呢+酒红，v10.4 新增 / v10.17.6 更名「扑克博弈」/ v10.17.10 8 件全彩 emoji 终版：4 花色+大王+花札+老虎机+骰子）
...
- `minD < 2.0`：**视觉混淆**，必须修正
- 设计意图同族系列（如 `titanium` 钛冷蓝 8 阶渐变 / `industrial` 铜锡渐变 / `lava` 熔岩单调 / `neonCity` 霓虹同调）属于刻意设计的"色阶皮肤"，免除该铁律
...
8 款不带 icon 的皮肤保持原 blockStyle 风格多样性（titanium metal / sakura glass / lava glossy / sunset glossy / dawn glossy / fantasy glass / neonCity neon / classic glossy）——无 icon 时不存在与 emoji 中心高光冲突问题。
...
- 暖奶油盘面 + 高饱和冷暖交替积木（**兼承接 v10.31 已剔除之 `macaroon` 法式马卡龙浅色需求**）
- 配色：`#B86858` `#5890D0` `#C8A060` `#489868` `#8868B0` `#489898` `#C06078` `#7068C8`（以 `web/src/skins.js` 为准）
...
| **v10.32** | **2026-04-30** | **剔除 `neural` 人工智能（36→35）**：带 icon 皮肤 28→27，全局 emoji 224→216；旧存档 `neural`→`neonCity`；i18n / `skinLore` / `companionStub` / 本文档同步；小程序镜像。 |
