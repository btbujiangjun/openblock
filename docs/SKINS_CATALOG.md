# OpenBlock 皮肤目录（37 款全集）


> 版本: v10.17.4 | 更新: 2026-04-29
...
OpenBlock 提供 **37 款** 主题皮肤，覆盖 4 大维度：

- **明暗底**：32 款深色 + 5 款浅色（`dawn` / `macaroon` / `pets` / `farm` / `desert`）
- **是否携带 emoji icon**：27 款带 icon（每款 8 枚）+ 10 款纯配色
...
合计 **37** 款。默认皮肤为 `titanium`（钛晶矩阵），由 `DEFAULT_SKIN_ID` 常量控制；玩家选择持久化至 `localStorage.openblock_skin`。
...
`classic`（中性灰盘） · `titanium`（钛冷蓝） · `cyber`（赛博暗紫黑） · `aurora`（北极冰蓝） · `neonCity`（都市夜近黑） · `ocean`（深海蓝） · `lava`（焦炭黑） · `koi`（深水蓝） · `bubbly`（深紫海） · `toon`（漫画聚光紫） · `pixel8`（街机厅黑） · `dawn`（晨光奶油） · `macaroon`（甜点暖白） · `food`（餐厅暖棕） · `music`（演出暗紫） · `pets`（家居米黄） · `universe`（深空近黑） · `beast`（荒野焦土） · `industrial`（焦煤铸铁） · `forbidden`（玄朱朱红） · `sports`（草绿球场） · `outdoor`（黎明山谷蓝，v10.17.4 新增即合规） · `vehicles`（机库灰混凝土） · `forest`（苔藓深绿） · `pirate`（深海舷板） · `farm`（浅春绿牧草） · `desert`（浅沙金） · `mahjong`（茶馆绿呢+实木暖灯，v10.17.3 重制） · `boardgame`（赌场红丝绒+绿呢+酒红，v10.4 新增 / v10.17.6 更名「扑克博弈」/ v10.17.10 8 件全彩 emoji 终版：4 花色+大王+花札+老虎机+骰子）
...
"
# 期望输出：总皮肤: 37 带 icon: 27 icon 总数: 216 重复: 0
```
...
# 输出：Synced 37 skins to miniprogram/core/skins.js
```
...
| 文件 | 作用 |
|---|---|
| `web/src/skins.js` | 全部 37 款定义（唯一事实源） |
