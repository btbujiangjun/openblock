...
OpenBlock 提供 **40 款** 主题皮肤，覆盖 4 大维度：

- **明暗底**：33 款深色 + 7 款浅色（`dawn` / `summer` / `cafe` / `toyBox` / `botanicalStudy` / `origamiPaper` / `winterCabin`）
...
合计 **40** 款。默认皮肤为 `titanium`（钛晶矩阵），由 `DEFAULT_SKIN_ID` 常量控制；玩家选择持久化至 `localStorage.openblock_skin`。
...
本批扩充以经典 UI 皮肤和主题丰富度为目标，使用许可清晰素材作为研究参考；方块主视觉优先使用外部 SVG UI 图标文件（`blockIconAssets`），`blockIcons` 仅保留为玩法规则 id、粒子/图鉴 fallback 与资源加载失败回退。素材来源与许可边界见 `docs/research/skins/SKIN_ASSET_SOURCES.md`。

外部 UI 图标约定：

- Web 资源：`web/public/assets/skins/<skinId>/block-0.svg` ... `block-7.svg`
- 小程序资源：`miniprogram/assets/skins/<skinId>/block-0.svg` ... `block-7.svg`
- `blockIconAssets` 与 `blockColors`、`blockIcons` 一一对齐，渲染层优先 `drawImage`，加载失败再回退 emoji。
- 新增 UI/材质型皮肤不得只依赖 emoji 表达主题；至少应提供 8 个同主题外部 UI 图标文件。
