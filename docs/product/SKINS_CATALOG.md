...
- **主题↔背景一致性**：盘面/页面背景的色相必须服务于皮肤主题叙事，而不仅仅是 icon 的反差衬底（详见 §2.4 与 §4 各小节）
...
**已下线与迁移**（`REMOVED_SKIN_ALIASES`，读档时自动改写）：`cyber`→`neonCity`，`macaroon`→`dawn`，`neural`→`neonCity`，`lava`→`sunset`。
...
`classic`（皇家蓝休闲消除盘） · `titanium`（钛冷蓝） · `aurora`（北极冰蓝） · `neonCity`（都市夜近黑） · `ocean`（深海蓝） · `koi`（深水蓝） · `bubbly`（深紫海） · `toon`（漫画聚光紫） · `pixel8`（街机厅黑） · `dawn`（晨光奶油） · `food`（餐厅暖棕） · `music`（演出暗紫） · `pets`（家居米黄） · `universe`（深空近黑） · `beast`（荒野焦土） · `industrial`（焦煤铸铁） · `forbidden`（玄朱朱红） · `sports`（草绿球场） · `outdoor`（黎明山谷蓝） · `vehicles`（机库灰混凝土） · `forest`（苔藓深绿） · `pirate`（深海舷板） · `farm`（浅春绿牧草） · `desert`（浅沙金） · `mahjong`（茶馆绿呢+实木暖灯） · `boardgame`（赌场红丝绒+绿呢+酒红）
...
全量审计：在 36 款皮肤中识别出 8 款带 icon 皮肤的 `minD < 2.0` 视觉混淆案例并全部修正，最终所有带 icon 的 26 款皮肤的 minD ≥ 2.0：
...
7 款不带 icon 的皮肤保持原 blockStyle 风格多样性（titanium metal / sakura glass / sunset glass / dawn cartoon / fantasy glass / neonCity neon / classic bevel3d）——无 icon 时不存在与 emoji 中心高光冲突问题。
...
- **定位**：休闲消除积木的标志性高饱和八色 + 中性深灰盘面，方块切换为 `bevel3d` 立体浮雕呈现挤出立方体观感
- **配色**（8 色）：`#3F6DD8` `#4FB8E8` `#52BC4B` `#FFC428` `#F5851E` `#A848E0` `#65C4F0` `#E84D5C`
- **盘面**：`gridOuter:#1C2630` / `gridCell:#2E3E50` / `cssBg:#141C24`（中性深灰）
- **风格**：`bevel3d` · `radius=4` · `gap=1`（圆润按钮光照模型：斜切边 — 顶 lighten 18% / 左 lighten 6% / 右 darken 16% / 底 darken 32%；中心面**对角渐变** TL lighten 18% → mid lighten 6% → BR 主色；左上 28%/22% 处叠加径向白色光斑（22% 中心、60% 半径迅速衰减）；中心面 TL 与顶斜切等亮度；**零描边、不楞、保留主色鲜艳度**）
- **icon**：—（纯配色）
