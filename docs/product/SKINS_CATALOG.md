...
OpenBlock 提供 **41 款** 主题皮肤，覆盖 4 大维度：

- **明暗底**：33 款深色 + 8 款浅色（`dawn` / `summer` / `cafe` / `toyBox` / `botanicalStudy` / `origamiPaper` / `winterCabin` / `inkGarden`）
- **是否携带 emoji / 符号 icon**：**40 款均带 icon**（每款 8 枚），新扩充主题允许使用 emoji 与短符号混合，以避开全局重复并提升经典 UI 语义。
- **blockStyle 渲染管线**：`glossy / metal / glass / cartoon / neon / jelly / flat / pixel8 / bevel3d` 共 9 种
- **全局 icon 唯一性**：40 × 8 = **320 个主题 icon 全部互不重复**（强约束 / 自动校验）
- **主题↔背景一致性**：盘面/页面背景的色相必须服务于皮肤主题叙事，而不仅仅是 icon 的反差衬底（详见 §二.4 与 §四各小节）
- **主题声效一致性**：每款现役皮肤必须在 `web/src/effects/skinSoundPalettes.js` 中声明 `SKIN_SOUND_THEMES`，由主题材质、根音、音阶和主题动机生成配套声效；Web 通过 `onSkinAfterApply` 随皮肤同步切换，小程序通过 `audioFx.setSkinTheme(skinId)` 重建程序化 WAV 音色
...
合计 **41** 款（含 §4.20 经典 UI / 材质扩充与 `inkGarden` 水墨雅集）。默认皮肤为 `titanium`（钛晶矩阵），由 `DEFAULT_SKIN_ID` 常量控制；玩家选择持久化至 `localStorage.openblock_skin`。
...
`mineralCave` / `circuitBoard` 早期把若干 `blockIcons` 槽位填成了 Unicode 纯色几何符号（色方块 / 色圆 / 小菱形），渲染后退化为「色块底 + 一个纯色几何形」，丧失主题叙事，与 §2.4 主题↔背景一致性铁律相冲。修正后两款均改用 **主题强相关、全局未占用** 的具象 emoji：

| 皮肤 | 旧 blockIcons（含纯色块） | 新 blockIcons | 主题增强叙事 |
|---|---|---|---|
| `mineralCave` 💍 宝石矿洞 | 💍 🔦 🧱 **🟫 🟪 🟨 🔸 🔹** | 💍 🔦 🧱 **⛏️ 🗿 🪔 🔨 🧊** | 宝石 → 矿灯 → 岩砖 → 采掘 → 遗迹 → 古灯 → 工具 → 晶体 |
| `circuitBoard` 🧲 电路矩阵 | 🧲 📶 **🟢 🟣 🟡 🟤** 🪛 🪫 | 🧲 📶 **🖥️ ⌨️ 🖱️ 📱** 🪛 🪫 | 磁信号 → 网络 → 主机 → 外设 → 移动端 → 工具 → 电源 |

约束验证：8 个新 emoji 全部全局未占用，280 emoji 互斥铁律仍成立；色板与盘面未动，仅替换 icon 槽位语义。判定方法：扫描 `blockIcons` 是否落入 Unicode 纯色块集合 `🟥🟧🟨🟩🟦🟪🟫⬛⬜ / 🔴🟠🟡🟢🔵🟣🟤⚫⚪ / 🔶🔷🔸🔹🔺🔻◆◇`，命中即视为「纯色配图」。
...
皮肤不仅改变视觉，也必须改变核心反馈声景。现役皮肤在 `web/src/effects/skinSoundPalettes.js` 中通过 `SKIN_SOUND_THEMES` 绑定声音主题，运行时由 `PRESETS` 生成 `place / clear / multi / combo / perfect / bonus / unlock / tick` 八类事件音色，并叠加原创主题动机；小程序端在 `miniprogram/utils/audioFx.js` 中使用同一组皮肤 profile 思路，切换皮肤时重建程序化 WAV 的 sound defs。

设计原则：

- **材质先行**：方块表面是什么，落子声就是什么。金属皮肤用短促金属/玻璃点击，海洋用水滴滑音，森林/咖啡用木质或叶片感，像素用短方波。
- **音阶承载叙事**：东方主题用五声音阶或东方音阶，魔法/宇宙用全音阶，庆典/糖果用明亮大调，暗黑/高压主题可用小调，但不做长期低频下坠。
- **动机贴近内容**：麻将要有瓷牌轻碰、木桌轻落或牌桌短摩擦，海洋要有气泡/水滴，森林要有叶噪/鸟鸣/轻虫声，紫禁城要有锣尾，像素/霓虹要有短方波，咖啡/美食要有木质杯盘轻敲。
- **氛围低打扰**：`web/src/effects/bgmStub.js` 已升级为程序化皮肤氛围层，首次交互后以低音量、低频率播放短环境动机；它补背景，不替代 SFX。
- **清脆明亮优先**：外部映射避免 `bong/close/minimize/back` 等低沉收束类素材；暗色/恶魔主题也只保留“张力”，不做长低频下沉。多消、长 combo、perfect、bonus 用同时起音的短和弦、玻璃闪光或材质尾音表达奖励，不使用琶音、分解和弦或连续阶梯上行音。程序化 fallback 不使用低通低频冲击层，非暗黑主题避免向下低频扫动。
- **外部素材内容相关**：麻将外部覆盖只用 Kenney Casino 中更柔和的 `cardPlace/cardSlide/chipLay/chipsStack/chipsHandle`，高频 `place/tick/select/clear` 禁止 `chipsCollide/diceThrow/dieThrow`；水域主题只用 `bubble/splash/water/rain`，常规落子禁用 `slime`；宠物/森林/侏罗纪使用 `cute/bug/ooh/nose` 等柔和短声，高频与奖励反馈禁用 `misc/bark/barking/howl/monster/roar/scream/hurt/cough/burp/snore/breath/burble/spit`，并排除 `poof/pfft/fart/deflate/balloon` 这类气球泄气或噗呲感来源；花仙/魔幻/魔法主题只使用 `pluck/glass/select/drop` 清亮素材，不使用 creature、宠物声、长空气噪声或 `confirmation/open` 这类阶梯式成功提示音。
- **切换同步**：`initSkinSoundPalettes({ audioFx })` 初始化当前主题，并订阅 `onSkinAfterApply`；皮肤转场在 `applyImmediate()` 后播放 `unlock`，因此解锁音会自动采用新主题。
- **配置完备**：新增 `SKINS[id]` 时必须新增 `SKIN_SOUND_THEMES[id]`。`tests/skinSoundPalettes.test.js` 会校验所有现役皮肤都有声音主题。
- **不在业务模块分叉**：玩法模块只发出 `audioFx.play('clear')` 等事件，不判断皮肤；皮肤差异由主题声效层统一处理。
- **外部素材边界**：调研测试可通过 `web/public/audio/skins/` 或 `miniprogram/assets/audio/skins/` 临时覆盖音效，系统会自动 fallback 到程序化声效；生产发布前必须替换为自制或授权素材。主题映射需通过 `docs/research/audio/audit_theme_audio.py` 审计，高频反馈不得命中刺耳、嘶哑、怪叫、不贴题、不清脆或不悦耳来源。
...
- **专属符号**：🦝🦅🐻 融合荒野动物意象，🪺 独占（避开 fairy 的 🍃、food 的 🥑）
...
## 4.20 经典 UI 与材质扩充（12 款，v10.34 新增）

本批扩充以经典 UI 皮肤和主题丰富度为目标。

> **⚠️ 占位资源回退记录**：本批 12 款（`arcadeCabinet` / `circuitBoard` / `toyBox` / `mineralCave` / `alchemyLab` / `botanicalStudy` / `spaceDock` / `dungeonLoot` / `origamiPaper` / `museumRelic` / `winterCabin` / `rainyWindow`）最初引用的 `blockIconAssets` SVG **是同一张占位图**（统一 `#7dd3fc` 蓝圆 + 白弧，仅 2–4 个英文字母标签不同），导致 12 款方块视觉几乎一致、彼此难分。已**移除这 12 款的 `blockIconAssets`**，回退到各自专属 emoji + `blockColors` 渲染——差异性由「全局唯一 emoji 集 + 专属色板 + 专属背景 + blockStyle（cartoon / neon / flat / pixel8）」承担，与 catalog 中其余 28 款 emoji 皮肤同一质量基线。占位 SVG 文件暂留仓库（未引用），待补真实美术后再恢复 `blockIconAssets`。

外部 UI 图标约定（当前仅 `inkGarden` 走真实 PNG 美术）：

- Web 资源：`web/public/assets/skins/<skinId>/block-0.{svg,png}` ... `block-7.*`
- 小程序资源：`miniprogram/assets/skins/<skinId>/block-0.{svg,png}` ... `block-7.*`
- `blockIconAssets` 与 `blockColors`、`blockIcons` 一一对齐，渲染层优先 `drawImage`，加载失败再回退 emoji。
- **新增 UI/材质型皮肤若声明 `blockIconAssets`，必须是真实差异化美术**（每款 8 张主题各异），不得用共用占位图；否则应省略该字段，直接走 emoji 渲染。素材来源与许可边界见 `docs/research/skins/SKIN_ASSET_SOURCES.md`。

| 皮肤 | 定位 | 视觉关键词 | 声效方向 |
|---|---|---|---|
| `arcadeCabinet` | 复古街机柜 | CRT 绿、投币橙、霓虹紫、像素 UI | pixel / arcade |
| `circuitBoard` | 电路板矩阵 | PCB 深绿、铜线、信号节点、外设端口（v10.34 去纯色 LED） | neon / device |
| `toyBox` | 玩具箱子 | 积木、拼图、游乐场软糖色 | cute / rubber |
| `mineralCave` | 宝石矿洞 | 岩层、晶簇、矿镐、石像、油灯、冰晶（v10.34 去纯色块） | crystal / stone |
| `alchemyLab` | 炼金工坊 | 药剂瓶、实验玻璃、铜绿台面 | magic / glass |
| `botanicalStudy` | 植物标本 | 标本纸、温室绿、浆果红 | nature / leaf |
| `spaceDock` | 太空船坞 | 轨道泊位、舱门、离子蓝灯 | cosmic / space |
| `dungeonLoot` | 地牢宝藏 | 暗石、宝箱、旧木、暗金 | dark / wood |
| `origamiPaper` | 折纸纸艺 | 纸纹、折线、柔色和风 | warm / paper |
| `museumRelic` | 博物遗珍 | 陶土、青铜、展柜暗金 | royal / relic |
| `winterCabin` | 冬日木屋 | 雪窗、暖炉、羊毛、松木 | wood / warm |
| `rainyWindow` | 雨窗夜灯 | 雨滴玻璃、街灯反射、蓝灰夜 | water / glass |

## 4.21 水墨雅集（1 款，inkGarden）

#### `inkGarden` 🖌️ 水墨雅集（浅色 · 外部 PNG 子图）

- **定位**：宣纸水墨文人雅趣——梅兰竹菊 + 文房四宝，淡彩晕染。
- **方块主视觉**：外部 PNG 子图 `web/public/assets/skins/inkGarden/block-0.png` … `block-7.png`（小程序、cocos 同名同步）；`blockIcons`（`💮🪻🎍💐🫧🪨🌰🖋️`）仅作玩法 id / 加载失败回退。
- **配色**（PNG 边缘同色相提亮占位）：`#9DAECE` `#B5A7CB` `#AAB091` `#C2AA88` `#9EB0B2` `#C5A5AB` `#9FAFBA` `#BAAC8B`
- **盘面**：`gridOuter:#E8E0D4` / `gridCell:#F5EFE6` / `gridLine:rgba(110,98,86,0.06)`（极弱网格）/ `gridGap:1` / `cssBg:#E8E0D4`，`uiDark:false`。
- **质感**：`blockStyle:cartoon` + `blockBevel`（轻顶光 / 底暗角 / 子图柔光叠加）+ `blockIconEnhance`（brightness 1.10 / contrast 1.06 / saturate 1.18）增强 PNG 通透感。
- **背景纹理**：`boardTexture:{ type:'xuanPaper', opacity:0.34, intensity:0.55 }` 程序化宣纸纤维，烘焙进 L0 离屏层；候选区（dock）由 `.block-dock` 的 CSS 宣纸噪声整体承担，**不逐槽填底色**（避免出现「候选格子」与切肤色块错位）。
- **水印**：基础 `🪭 📜`，`opacity:0.11` / `scale:0.26`；HD 换装 `⛰️ 🎑 🍶 🌫️ 🏞️`（5 件全局唯一，避开 forbidden 的 🎐 与 pixel8 的 🏯）。
- **声效**：`SKIN_SOUND_THEMES.inkGarden`（五声音阶 / 宣纸落笔感），小程序 `audioFx` 同步重建。

## 五、icon 全局唯一性矩阵

### 5.1 40 款带 icon 皮肤完整 icon 列表（共 320 个）
...
| `cyberpunk` | 🤖 💻 🔌 📡 🧬 📊 🔋 💾 |
| `fiesta` | 🎉 🎊 🎈 🪅 🥳 🎁 🧨 🎪 |
| `mineralCave` | 💍 🔦 🧱 ⛏️ 🗿 🪔 🔨 🧊（v10.34 修正纯色块）|
| `circuitBoard` | 🧲 📶 🖥️ ⌨️ 🖱️ 📱 🪛 🪫（v10.34 修正纯色 LED）|
...
"
# 期望输出：总皮肤: 40 带 icon: 40 icon 总数: 320 重复: 0
```
...
| 矿洞采掘 | mineralCave | 💍🔦🧱⛏️🗿🪔🔨🧊（v10.34 去纯色块，主题叙事化）|
| 电子外设 | circuitBoard | 🧲📶🖥️⌨️🖱️📱🪛🪫（v10.34 去纯色 LED，叙事补全 PC 外设链）|
...
# 输出：Synced 41 skins to miniprogram/core/skins.js
```
...
## 十、方块 icon 全量池：利用情况与表意匹配

> 代码事实源：`web/src/skins.js`  
> 硬约束：**40 款带 `blockIcons` × 8 = 320 枚主题 icon，跨皮肤全局互斥**。
...
|------|-----|
| 带 icon 皮肤数 | 40 |
| 方块 icon 总数 | **320**（40 × 8，利用率 100%，无闲置槽位） |
| 纯配色皮肤 | 0（当前 40 款均有主题 icon） |
