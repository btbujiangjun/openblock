...
- **HD 模式 emoji 换装（v1.49 新增 · v4 终版定稿）**：
  v1.49 v4 终版**全量 34 个皮肤**都注入 **5 件** hdIcons（= 默认锚点数 5），每个皮肤主题强相关 emoji，**全局 170 件唯一**且不与任何皮肤的基础 icons 重叠。完整设计表见 [`CHANGELOG.md` v1.49 §B](../../CHANGELOG.md)。

  **核心修复（"水印图片不得重复"）**：v3 的 hdIcons 数量 = 基础 icons 数量（多为 2-4 件），但默认锚点数是 5 → `icons[i % length]` 在 `length < 5` 时必然出现重复 emoji（mahjong 2 件套在 5 锚点上 i%2 循环 = 3 个 🎲 + 2 个 🀐）。v4 把所有皮肤的 hdIcons 数量**统一抬到 5 件**，使盘面 5 个水印 emoji**两两不同**。

  mahjong 的 v4 hdIcons 是 **`['🎲', '🀐', '🀙', '🀇', '🀄']`**：
  - `🎲` 骰子 — 摇骰开局的灵魂道具
  - `🀐` 一索 — 幺鸡 / 港粤"打雀"的雀
  - `🀙` 一筒 — 筒子门「一」头牌代表
  - `🀇` 一万 — 万子门「一」头牌代表
  ...
  **亮度 / scale / 运动模式与所有皮肤完全一致**：仅替换 emoji，继承基础 opacity 0.10、走默认 5 锚点 + 默认 scale + 同一 segment 时长，与所有皮肤共享同一漂浮节奏（即 §A 的 Catmull-Rom 滑动窗口 + 换皮不换轨契约）。

  | v1.49 mahjong 演进 | 方案 | 回退原因 |
  |---|---|---|
  | v1 | 6 件套 + 自定义 6 锚点 | 破坏与其他皮肤一致的"5 锚点漂浮节奏" |
  | v2 | 3 件套 + `hdOpacity 0.13`（vs dawn 0.12 是其他皮肤最高） | 亮度高于所有皮肤 |
  | v3 | 2 件套 + 仅覆盖 hdIcons | 5 锚点 i%2 循环 → 3 个 🎲 重复（用户截图证实） |
  | **v4 当前** | **5 件套（=锚点数）+ 仅覆盖 hdIcons** | 盘面 5 个水印两两不同；亮度 / scale / 锚点 / 节奏全部继承基础值，与所有皮肤完全对齐；终版扩展为全量 34 皮肤通用模板 |

  配置见 `web/src/skins.js → mahjong.boardWatermark.{ icons, opacity, hdIcons }`，渲染契约见 [`CHANGELOG.md` v1.49](../../CHANGELOG.md)。

  **同一版本 v1.49 还重写了所有皮肤共享的"水印漂浮"算法**（影响 mahjong 在内的全部 HD 水印）：从旧"每帧 dt-ease"（dt 16-240ms 跳变 → 抖动）→ 中间方案"wall-time + smootherstep 段插值"（消除 dt 抖动但段头尾静止 + wobble 仍显原地抖）→ **当前方案"Catmull-Rom spline 滑动窗口"**（段端点切线 `(p2-p0)/2` 与 `(p3-p1)/2` 天然连续且非零，icon 持续在动无停顿，自然弯曲取代 wobble）。

  **「换皮不换轨」契约（v1.49 修订）**：`drift.key` 公式去掉 `skin.id` —— 所有同 5 锚点皮肤（mahjong / sakura / aurora / pixel8 等）共享**同一漂浮时间线**，切换皮肤时 icon 继续从当前位置漂浮、不重置回锚点，仅 emoji 字符替换。这从代码层面保证了"麻将水印的运动轨迹 ≡ 其他皮肤水印的运动轨迹"，而不仅仅是"算法相同"。详见 [`CHANGELOG.md` v1.49](../../CHANGELOG.md) §A。
