...
> 配套文档：[`MONETIZATION.md`](./MONETIZATION.md)（商业化权威入口）· [`MONETIZATION.md`（商业化策略定制指南）](./MONETIZATION.md#十四商业化策略定制指南)
> 实现：`web/src/monetization/monPanel.js` + `backend/monetization_backend.py`
> 系统全景、模型公式、API 与上线边界以 `MONETIZATION.md` 为准。
...
### 一、设计
1. [面板的商业理由](#11-面板的商业理由)
2. [信息架构与设计原则](#12-信息架构与设计原则)
3. [角色画像与典型动作](#13-角色画像与典型动作)

### 二、原理
4. [鲸鱼分模型](#21-鲸鱼分模型从原始指标到分群标签)
5. [六个信号的语义与计算](#22-六个信号的语义与计算)
6. [规则引擎](#23-规则引擎从上下文到推荐动作)
7. [缓存层级与配置传播链](#24-缓存层级与配置传播链)

### 三、策略
8. [内置策略矩阵的商业解读](#31-内置策略矩阵的商业解读)
9. [运营调参 PlayBook](#32-运营调参-playbook)
10. [A/B 实验与漏斗分析](#33-ab-实验与漏斗分析)

### 四、内容
11. [Tab 1：总览（全局健康仪表）](#41-tab-1总览全局健康仪表)
12. [Tab 2：用户画像（个体诊断）](#42-tab-2用户画像个体诊断)
13. [Tab 3：模型配置（参数调节台）](#43-tab-3模型配置参数调节台)
14. [Tab 4：功能开关（开关箱）](#44-tab-4功能开关开关箱)

### 五、工程
15. [扩展指南](#51-扩展指南)
16. [故障排查与调试](#52-故障排查与调试)
17. [附录：字段速查表](#53-附录字段速查表)
...
#### 1.2.1 4 Tab 布局背后的"看 → 分析 → 调 → 控"四步法
...
### 2.1 鲸鱼分模型：从原始指标到分群标签

#### 2.1.1 为什么不直接用"消费金额"分群？
...
#### 2.2.2 实时信号三件套（`PlayerProfile` → `realtimeSignals`）
...
> 在线客户端会在下次 `fetchPersonaFromServer` 时通过 `data.model_config` 字段拉到，但这是"被动同步"。如需"主动推送"配置变更（WebSocket / SSE），见 §5.1 扩展指南。
...
#### 5.3.1 cursor:help 字段全表（51 项）

| 区域 | help key | 默认值 / 含义 |
|------|---------|--------------|
| **Tab 1 KPI** (6) | `kpi.{total_users / dau_7d / games_7d / avg_score_30d / avg_session_30d / lb_participants}` | 见 §4.1.2 |
| **Tab 1 分群** (3) | `segment.{whale / dolphin / minnow}` | minWhaleScore = 0.60 / 0.30 / 0.00 |
| **Tab 2 信号** (6) | `signal.{segment / activity / skill / frustration / nearMiss / flow}` | 见 §2.2 |
...
| **Tab 4 Flag** (10) | `flag.{adsRewarded / adsInterstitial / iap / dailyTasks / leaderboard / skinUnlock / seasonPass / pushNotifications / replayShare / stubMode}` | 见 §4.4.1 |
...
| [`MONETIZATION.md`（商业化策略定制指南）](./MONETIZATION.md#十四商业化策略定制指南) | 三种定制粒度；本文档对应粒度 A（运营调参） |
