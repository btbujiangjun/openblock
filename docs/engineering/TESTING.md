...
```bash
npx vitest run tests/spawnEvaluation.test.js
npx vitest run tests/decisionFlowVizDriverPaths.test.js tests/decisionFlowVizPerf.test.js
npm run spawn:eval -- --sessions 1 --max-steps 20 --max-triplets 24 \
  --strategies normal --policies survival --spawn-generators budget-p2 \
  --personalization 0.15 --temperature 0.05 --surprise-gain 0.1 --surprise-cooldown 6
```

若改动涉及 Web Worker / 可视化页 / SQLite 方案保存，额外跑 `npm run build`，并手动打开 `/spawn-eval.html` 验证页面默认不自动评估、点击后不阻塞 UI。
