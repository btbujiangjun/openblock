...
| `adaptiveSpawn.js` | 自适应出块策略引擎：**10 信号融合** → stress → 10 档 profile 插值 + spawnHints。详见 **[`ADAPTIVE_SPAWN.md`](../algorithms/ADAPTIVE_SPAWN.md)** |
...
**后端**：`backend/monetization_backend.py`（Flask Blueprint）提供 `/api/mon/*` 路由：  
