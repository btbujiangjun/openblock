...
1. [出块策略定制](#一出块策略定制)
2. [Stress 信号定制](#二stress-信号定制)
3. [难度模式定制](#三难度模式定制)
4. [广告策略定制](#四广告策略定制-ad-sdk)
5. [IAP 策略定制](#五iap-策略定制-iap-sdk)
6. [商业化分群策略](#六商业化分群策略)
7. [个性化推荐策略](#七个性化推荐策略)
8. [RL 训练策略](#八rl-训练策略)
9. [玩家成长策略](#九玩家成长策略)
...
| 11 | **友好盘面救济**＼� | `friendlyBoardRelief` | [−0.18, 0] | 盘面 holes=0 + 临消行/多消候选/清屏机会充沛 + 节奏处于 payoff 时主动减压，让拟人化压力表与玩家直觉同向 |
