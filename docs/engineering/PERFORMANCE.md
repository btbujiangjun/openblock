...
> **配套文档**：[性能基线与回归检测](#十三性能基线与回归检测) —— 本文讲"怎么优化"，那篇讲"优化后怎么不被人无声打破"。CPU 基线 `npm run perf:check`、GPU 基线 `window.__perfOverlay.startProfile(10)`。

---

## 一、脏区域追踪 (Dirty Rect Tracking)
...
### 6.3 `loadProgress` 缓存（`progression.js`）

- 内存缓存 + localStorage 回退

### 6.4 页面可见性（`lib/pageVisibility.js`）

- 后台标签页暂停定时器

### 6.5 首屏后懒加载（`main.js` + `initDeferredPanels.js`）
...
### 6.8 `loadProgress` 缓存（`progression.js`）
...
### 6.9 页面可见性（`lib/pageVisibility.js`）
...
### 6.10 首屏后懒加载（`main.js` + `initDeferredPanels.js`）
...
### 6.12 商业化模块的 LAZY_MODULES 机制（v1.12）
...
## 八、决策数据流面板 GPU 合成层第二轮瘦身 (v1.55.2)
...
## 九、HUD 常驻 CSS 动画 GPU 合成瘦身 (v1.55.3)
...
---
## 十一、celebrate toast 常驻动画泄漏与 backdrop-filter 清理 (v1.55.5)
...
- [宝箱与钱包](../product/CHEST_AND_WALLET.md)
- [测试指南](./TESTING.md)
...
### 13.8 已落地的 GPU 优化（v1.55.10 - v1.55.13）
...
- [前端性能优化说明](./PERFORMANCE.md) —— 性能设计原则与已落地优化清单
...
- [出块架构与算法](../algorithms/ALGORITHMS_SPAWN.md#十二出块算法架构总览工程分层) —— `generateDockShapes` 实际职责
