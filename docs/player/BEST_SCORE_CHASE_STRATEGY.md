...
| PB 段 | 触发条件 | 音频文件 | 播放策略 |
|------|----------|----------|----------|
| D2 临近 | `0.80 ≤ score / runStartPB < 0.95` | `pb_near.wav` | 约 3 秒低音量庆祝/期待 cue，进入 PB 区时播放一次。 |
| D3 决战 | `0.95 ≤ score / runStartPB ≤ 1.0` | `pb_sprint.wav` | 约 3 秒更亮的冲刺 cue，进入临门一脚时播放一次。 |
| D4 突破 | `score > runStartPB` 首次命中 | `pb_release.wav` | 约 3 秒经典庆祝 release cue，破 PB 后播放一次。 |

跨端资源路径统一为 `audio/game/pb_chase/`，对应 Web、小程序、Cocos 三端资源目录。该层共享现有音效开关；关闭音效时 PB cue 静默。`runStartPB` 必须使用开局快照，不能用局内实时抬升后的 `bestScore`，否则破 PB 后会丢失 release 判定。
...
#### §4.17.x 系统性优化 五项系统性优化（v1.68，✅ 已落地）

> 📍 **修复目标**：commit `8ff29f4f` 首版相对论上线后 4 类体感回退——新手出块碎/高 PB 早期得分慢/构造式爽消被对齐评分挑掉/温暖局策略被覆盖。**五项系统性优化 不是开关**，是相对论与既有相位/状态机的架构级耦合软化。

| 编号 | 名称 | 解决问题 | 关键改动 | 配置位 |
|---|---|---|---|---|
| **相位化对齐预算** | 相位化对齐预算 `relativityIntent` | 顺玩家相位（harvest/warmup/pb_chase）被对齐评分挑掉爽消 | 4 档分级：`off / prior_only / kbest_only / full`。bypass + recovery/onboarding/bottleneck/near_miss → off；harvest/warm/warmup/pb_chase/release → prior_only；其它 full | `difficultyRelativity.js :: resolveRelativityIntent` |
| **相位化几何增益** | 相位化几何信号增益 `phaseGeomGain` | 新手/温暖局期 1 个 close1 就把 ability riskLevel 拉高、形状漂向 t/z | onboarding=0.3 / warmRun=0.5 / 默认 1.0。仅衰减负向项（holePenalty/nearClearScore/lockRiskScore），正向 spatialPlanning 不变 | `game_rules.json :: adaptiveSpawn.phaseGeomGain` |
| **PEOG 抗抖动** | PEOG bottleneck/near_miss 延迟让位 | 瞬时几何谷值打断 PB 加压窗口 | `_bottleneckHits / _nearMissHits` 累计，连续 ≥ 阈值（默认 2）才 `_bypassNow`；信号消失立即归零 | `pbChase.earlyOvershootGuard.{bottleneckYieldHits, nearMissYieldHits}` |
| **difficultyVec 真实信号化** | `difficultyVec` 真实化 | 6 维客观难度被旧 5 项 term 平均，clearEff/recovery/combo 与玩家爽点 / 回收能力错位 | 新增 `clearPotential / cleanPath / permVariance`（由 `solutionMetrics` 派生），`projectDifficultyVector` 缺省 term 自动从加权和剔除（向后兼容） | `spawnStepDifficulty.vectorWeights` + `difficultyRelativity.shapePrior.dimAffinity` |
| **b* 前期上界** | b\* 早期上界 `earlyPhaseBStarCap` | 高 PB 玩家局初被高 θ 拉到客观偏难，得分慢、进不去 PB 调整状态 | 低 d\* 阶段（< `earlyPhaseDStar=0.40`）把任一维 b\* 钳制在 `d + earlyPhaseBStarCap=0.10` 以内；中后段自动让位 | `adaptiveSpawn.difficultyRelativity.{earlyPhaseDStar, earlyPhaseBStarCap}` |

**优先级（同帧多生效时的语义层级，由高到低）**：
1. 硬约束/救济链（不变，**始终最高**）；
2. PEOG 抗抖动 PEOG 延迟让位（达到累计阈值时直接 bypass 相对论）；
3. b* 前期上界 b\* 早期上界（在 b\* 公式内钳制）；
4. 相位化对齐预算 intent 门控（决定是否走 shapePrior / best-of-K）；
5. 相位化几何增益 phaseGeomGain（影响 ability 输入，间接影响 b\*/intent）；
6. difficultyVec 真实信号化 difficultyVec 升级（影响 best-of-K 评分尺度）。

**回归红线（与 §4.17 主条款共享 + 新增）**：① 相位化几何增益 全 1.0 时 ability 行为完全等价旧版（向后兼容）；② PEOG 抗抖动 单帧瞬时信号不让位（hits 计数器被信号消失重置）；③ difficultyVec 真实信号化 `solutionMetrics.truncated=true` 时新 term 全 null，scalar `stepDifficulty` fixture 不变；④ b* 前期上界 高 d\* 阶段（≥ `earlyPhaseDStar`）b\* 不受钳制；⑤ 相位化对齐预算 `off` 档下完全等价"未启用相对论"行为。
