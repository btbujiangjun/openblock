...
- **Adaptive Spawn Engine** — multi-signal stress fusion drives multi-axis spawn targets for real-time difficulty adjustment
...
| **Difficulty Mode** | `adaptiveSpawn.difficultyTuning` + `adaptiveSpawn.profiles` | [Strategy Guide](./docs/engineering/STRATEGY_GUIDE.md#3-难度模式定制) |

### Quick Start

**Prerequisites**: Node.js 18+, Python 3.10+. Mobile packaging uses Capacitor; Node.js 22+ is recommended for Android/iOS shell commands.
...
**Android / iOS shell**:
```bash
npm run mobile:build # Build Web assets and sync Capacitor platforms
...
| [docs/engineering/TESTING.md](./docs/engineering/TESTING.md) | Test strategy, regression matrix, validation commands |
...
- **自适应出块引擎** — 多信号 stress 融合 + 多轴出块目标，实时调节难度、消行机会、解空间与节奏
...
**出块引擎**
- 三层架构：规则约束（Layer 1）→ 体验优化（Layer 2）→ 局间弧线（Layer 3）
- 多信号 stress 融合：分数压力 / 连战加成 / 技能调节 / 心流 / 节奏 / 恢复 / 挫败救济 / Combo 奖励 / 趋势 / 盘面风险救济
- 多轴出块目标：形状复杂度 / 解空间压力 / 消行机会 / 空间压力 / payoff 强度 / 新鲜度
...
├── miniprogram/            # 微信小程序适配
├── mobile/                 # Android / iOS Capacitor 客户端外壳
├── docs/                   # 技术文档（在线：/docs）
...
详细指南见 [docs/engineering/DEV_GUIDE.md](./docs/engineering/DEV_GUIDE.md)，快速参考：
...
// 自定义出块策略
// 修改 shared/game_rules.json 中的 adaptiveSpawn.profiles / signals / difficultyTuning
// 即可调整 stress、多轴出块目标和难度档位，无需修改引擎代码
```
