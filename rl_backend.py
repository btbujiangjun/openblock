...
环境变量（可选）:
  ...
  RL_DEVICE           auto | cpu | mps | cuda | cuda:N（多卡训练见 RL_CUDA_DEVICE_IDS；Flask 推理单卡）
  RL_LR               Adam 学习率，默认 1e-3（light 模型）；旧重模型建议 3e-4
  RL_GAMMA            折扣因子，默认 0.99
  RL_VALUE_COEF       价值损失权重，默认 0.25
  RL_VALUE_HUBER_BETA smooth_l1 的 beta，默认 10
  ...
  RL_RETURN_SCALE     蒙特卡洛回报缩放（仅训练用），默认 0.1；设 1 关闭
...
def _effective_entropy_coef(global_episode: int) -> float:
    ...
    base = float(os.environ.get("RL_ENTROPY_COEF", "0.01"))
...
def _default_meta():
    ...
    return {
        "width": int(os.environ.get("RL_WIDTH", "64")),
...
def _ensure_initialized():
    ...
    with _rl_lock:
        ...
        lr = float(os.environ.get("RL_LR", "1e-3"))
...
def create_rl_blueprint() -> Blueprint:
    ...
    def rl_train_episode():
        ...
        value_coef = float(data.get("value_coef", os.environ.get("RL_VALUE_COEF", "0.25")))
        value_huber_beta = float(os.environ.get("RL_VALUE_HUBER_BETA", "10"))
