...
环境变量（可选）:
  ...
  RL_LR               Adam 学习率，默认 3e-4（与 rl_pytorch.train 新默认一致；旧 checkpoint 可仍用 1.5e-4）
  ...
  RL_RETURN_SCALE     蒙特卡洛回报缩放（仅训练用），默认 0.032，使 V(s) 与 smooth_l1 更稳；设 1 关闭
...
def _default_meta():
    ...
    return {
        ...
        "arch": os.environ.get("RL_ARCH", "shared"),
    }
