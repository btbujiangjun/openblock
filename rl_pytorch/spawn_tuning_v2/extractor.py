...
def pb_aware_d_pb_base(ratio: float) -> float:
    """业务 S 形基础难度 — 直接复用 ideal target_S_curve (4 段分段).

    sample d_step = pb_aware_d_pb_base(r) + state_offset(±0.10),
    平均后 sample d_curve ≈ target_S_curve, model 直接学到 ideal target.

    范围 [D_BASE, D_CAP] = [0.20, 1.00].
    """
    # 延迟导入避免循环依赖 (target_curve 不引 extractor)
...
class StepInfo:
    ...
    def step_difficulty(self, prev_fills: Sequence[float], ratio: float = 0.0) -> float:
        ...
        v2.12: d_pb_base 直接复用 ideal target_S_curve (4 段分段),
        state_offset 提供 ctx 差异性 (±0.10).

          d_step = clip(target_S_curve(r) + 0.20 * (state_d - 0.5), 0, 1)
        """
        if self.no_move:
            return 1.0
        # state_d (棋盘状态难度)
        ...
        # v2.12: d_pb_base = ideal target_S_curve (跟训练目标完全一致)
        d_pb_base = pb_aware_d_pb_base(ratio)
