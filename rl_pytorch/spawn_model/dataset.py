...
# v1.68（对齐预算/几何增益/前期上界）：DIM 72 → 78。尾部新增 6 维相对论运行时配置：
...
#   [77]    earlyPhaseCapHit（b* 前期上界 b* 早期上界触发标志，0/1），缺省 → 0。
...
# 相位化对齐预算：relativityIntent one-hot 键序（必须与 web/src/spawnModel.js _RELATIVITY_INTENT_KEYS 一致）。
_RELATIVITY_INTENT_KEYS = ['off', 'prior_only', 'kbest_only', 'full']


def _relativity_intent_one_hot(intent):
    """相位化对齐预算：4 维 one-hot。null/未知 → 全 0（中性）。"""
    out = [0.0, 0.0, 0.0, 0.0]
